...
begin
  if to_regclass('public.app_environment') is null then
    raise exception 'REFUSING: suspension-admin production install requires public.app_environment';
  end if;
  if exists (select 1 from public.app_environment) then
    raise exception 'REFUSING: suspension-admin production install requires the empty production environment marker';
  end if;
  if to_regclass('public.forum_install_state') is null
     or (select count(*) from public.forum_install_state) <> 1 then
    raise exception 'REFUSING: suspension-admin production install requires the reviewed Forum v1 production baseline';
  end if;
  if to_regclass('public.profiles') is null
    ...
    raise exception 'REFUSING: suspension-admin production install Forum v1 objects are incomplete';
  end if;
  if not exists (
    select 1 from public.forum_settings where id = true and mode = 'off'
  ) then
    raise exception 'REFUSING: suspension-admin production install requires forum mode off';
  end if;
  if to_regprocedure(
       'public.forum_stage_prepare_suspension_admin_fixtures(text,uuid[])'
     ) is not null then
    raise exception 'REFUSING: suspension-admin production install found a staging fixture helper';
  end if;
  if to_regprocedure('public.forum_admin_set_suspension_by_username(text,integer,text)') is not null
     or to_regprocedure('public.forum_admin_list_suspensions()') is not null then
    raise exception 'REFUSING: suspension-admin production install suspension-admin wrappers already exist';
  end if;
end;
$forum_suspension_admin_production_guard$;

create temporary table forum_suspension_admin_production_install_baseline
...
select
  ...
  (select count(*) from public.profiles) =
    (select profile_rows from forum_suspension_admin_production_install_baseline)
    as profiles_unchanged,
  (select count(*) from public.forum_posts) =
    (select post_rows from forum_suspension_admin_production_install_baseline)
    as posts_unchanged,
  (select count(*) from public.forum_comments) =
    (select comment_rows from forum_suspension_admin_production_install_baseline)
    as comments_unchanged,
  (select count(*) from public.forum_votes) =
    (select vote_rows from forum_suspension_admin_production_install_baseline)
    as votes_unchanged,
  (select count(*) from public.forum_user_stats) =
    (select user_stat_rows from forum_suspension_admin_production_install_baseline)
    as user_stats_unchanged,
  (select count(*) from public.forum_reports) =
    (select report_rows from forum_suspension_admin_production_install_baseline)
    as reports_unchanged,
  (select count(*) from public.forum_moderation_log) =
    (select moderation_log_rows from forum_suspension_admin_production_install_baseline)
    as moderation_log_unchanged,
  (select count(*) from public.forum_suspensions) =
    (select suspension_rows from forum_suspension_admin_production_install_baseline)
    as suspensions_unchanged,
  (select count(*) from public.forum_rate_events) =
    (select rate_event_rows from forum_suspension_admin_production_install_baseline)
