...
begin
  if to_regclass('public.app_environment') is null then
    raise exception 'REFUSING: forum production audit requires public.app_environment';
  end if;
  if exists (select 1 from public.app_environment) then
    raise exception 'REFUSING: forum production audit requires the empty production environment marker';
  end if;
  if to_regclass('public.profiles') is null
    ...
    raise exception 'REFUSING: forum production audit identity/admin dependencies are incomplete';
  end if;
  if not exists (select 1 from pg_roles where rolname = 'anon')
     or not exists (select 1 from pg_roles where rolname = 'authenticated')
     or not exists (select 1 from pg_roles where rolname = 'service_role') then
    raise exception 'REFUSING: forum production audit browser/service roles are incomplete';
  end if;
  ...
  if cardinality(existing) > 0 then
    raise exception 'REFUSING: forum production audit found existing forum objects: %',
  ...
  if to_regprocedure('public.forum_mode()') is not null
    ...
    raise exception 'REFUSING: forum production audit found existing forum functions or indexes';
  end if;
  if exists (
    ...
    raise exception 'REFUSING: forum production audit cannot safely snapshot PUBLIC profile write grants';
  end if;
  if exists (
    select 1 from (
      select lower(btrim(username))
    ...
    raise exception 'REFUSING: forum production audit found case-insensitive username collisions';
  end if;
end;
