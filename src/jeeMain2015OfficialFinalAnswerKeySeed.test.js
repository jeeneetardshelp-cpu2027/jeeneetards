...
const seed = readFileSync(
  "docs/sql/study_materials_jee_main_2015_official_final_answer_key_seed_2026-09-04.sql",
  "utf8",
);
const manifest = JSON.parse(readFileSync(
  "docs/study-materials/jee-main-2015-official-final-answer-key-manifest.json",
  "utf8",
));
...
describe("JEE Main 2015 official final answer-key package", () => {
  it("contains the complete official Paper 1 final answer key", () => {
    ...
    expect(manifest.verification).toContain("All three pages");
    ...
    expect(resource.title).toBe("JEE Main 2015 Final Answer Key (Paper 1 B.E./B.Tech)");
    expect(resource.officialFilename).toBe("Final Answer Key JEE 2015.pdf");
    expect(resource.offlineHeading).toContain("OFFLINE EXAMINATION");
    expect(resource.onlineHeading).toContain("ONLINE EXAMINATION");
    expect(resource.offlineExamDate).toBe("2015-04-04");
    expect(resource.offlineSets).toEqual(["A", "B", "C", "D"]);
    expect(resource.onlineExamDates).toEqual(["2015-04-10", "2015-04-11"]);
    expect(resource.pageCount).toBe(3);
    ...
    expect(resource.sha256).toBe("7F19B58CB73CB3D0F753A517D5A477D02107D82AF0DC89072F702E615796AF09");
  ...
  it("keeps Paper 2, response sheets and unofficial solutions out", () => {
    expect(manifest.exclusions).toContain("candidate response sheets");
    ...
    try {
      ...
      const materials = await db.query(`
        select count(*)::integer as count
         ...
         where title = 'JEE Main 2015 Final Answer Key (Paper 1 B.E./B.Tech)'
      ...
      const scopes = await db.query(`
        select count(*)::integer as count
         ...
         where m.title = 'JEE Main 2015 Final Answer Key (Paper 1 B.E./B.Tech)'
           and g.slug = 'jee'
