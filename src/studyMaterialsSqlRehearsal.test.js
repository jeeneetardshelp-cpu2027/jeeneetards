...
const jeeMain2014PapersSeed = readFileSync(
  "docs/sql/study_materials_jee_main_2014_papers_seed_2026-08-13.sql",
  "utf8",
);
const jeeMain2015PapersSeed = readFileSync(
...
const neetUg2024PapersSeed = readFileSync(
...
const neetUg2025PapersSeed = readFileSync(
...
describe("study materials v1 local SQL rehearsal", () => {
  ...
  it("loads all eight official JEE Main 2014 papers into JEE-only exam scopes", async () => {
    const pg = await productionShapedDatabase();
    try {
      await pg.exec(jeeMain2014PapersSeed);
      await pg.exec(jeeMain2014PapersSeed);
      const counts = await pg.query(`
        select
          (select count(*)::integer from public.study_materials) as materials,
  ...
  it("loads all six official JEE Main 2015 papers into JEE-only exam scopes", async () => {
    const pg = await productionShapedDatabase();
    try {
      await pg.exec(jeeMain2015PapersSeed);
      await pg.exec(jeeMain2015PapersSeed);
      const counts = await pg.query(`
        select
          (select count(*)::integer from public.study_materials) as materials,
  ...
  it("loads two official NEET UG 2024 papers into NEET-only exam scopes", async () => {
    const pg = await productionShapedDatabase();
    try {
      await pg.exec(neetUg2024PapersSeed);
      await pg.exec(neetUg2024PapersSeed);
      const counts = await pg.query(`
        select
          (select count(*)::integer from public.study_materials) as materials,
      ...
      expect(neet.rows).toHaveLength(2);
      expect(Number(neet.rows[0].total_count)).toBe(2);
      expect(neet.rows.every((row) => row.language === "English")).toBe(true);
      expect(neet.rows.every((row) => row.exam_year === 2024)).toBe(true);
  ...
  it("loads four official NEET UG 2025 papers into NEET-only exam scopes", async () => {
    const pg = await productionShapedDatabase();
    try {
      await pg.exec(neetUg2025PapersSeed);
      await pg.exec(neetUg2025PapersSeed);
      const counts = await pg.query(`
        select
          (select count(*)::integer from public.study_materials) as materials,
      ...
      expect(Number(neet.rows[0].total_count)).toBe(4);
      expect(neet.rows.every((row) => row.language === "English")).toBe(true);
      expect(neet.rows.every((row) => row.exam_year === 2025)).toBe(true);
      expect(neet.rows.every(
        (row) => row.source_name === "National Testing Agency (NEET UG)",
      )).toBe(true);
      expect(neet.rows.every((row) => row.scopes.length === 1)).toBe(true);
