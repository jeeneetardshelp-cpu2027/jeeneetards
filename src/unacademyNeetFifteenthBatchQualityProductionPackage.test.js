...
const sqlPath = "docs/sql/unacademy_neet_fifteenth_batch_quality_review_2026-08-06.sql";
const readinessPath = "docs/unacademy-neet-fifteenth-batch-quality-readiness-2026-08-06.md";
const sql = readFileSync(sqlPath, "utf8");
const readiness = readFileSync(readinessPath, "utf8");
const expectedHash = "11ae116a88ca0da76433406c91c44cb308282d77354d3d084c2af9bac959a173";
...
const seedProductionShape = async () => {
  ...
  await pg.exec(`
    ...
    insert into public.playlists values
      (423, 'NEET: Alcohols, Phenols & Ethers | Class 12 | Unacademy NEET | Anoop V.', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Anoop Vashishtha', 'PLsgHooHkqhhNnQ7F6-Wfril1wn1_JrWNP', 147, 2, 2,
       array['12th'], '12th'),
      (424, 'Fluid Mechanics -  Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Physics | Mahendra Singh', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Mahendra Singh', 'PLsgHooHkqhhMMPfEYr7m_ofP61K_YScyw', 147, 2, 1,
       array['11th'], '11th'),
      (425, 'Kinematics 1D -  Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Physics | Mahendra Singh', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Mahendra Singh', 'PLsgHooHkqhhM5m3xbTdZ2cDX8S_22jdSX', 147, 2, 1,
       array['11th'], '11th');

    insert into public.videos select n, 1 from generate_series(1, 4683) n;
    ...
    update public.chapters set name = 'Organic Compounds Containing Oxygen', subject_id = 2 where id = 92;
    update public.chapters set name = 'Mechanical Properties of Fluids', subject_id = 1 where id = 26;
    update public.chapters set name = 'Kinematics', subject_id = 1 where id = 1;
    ...
    insert into public.playlist_teachers
    select n, 1 + ((n - 500) % 30), 'instructor', 1 from generate_series(500, 575) n;
    insert into public.playlist_teachers values
      (423, 36, 'instructor', 1), (424, 34, 'instructor', 1), (425, 34, 'instructor', 1);
    insert into public.playlist_quality_reviews
      (playlist_id, before_state, after_state, note)
    select n, '{}'::jsonb, '{}'::jsonb, 'existing review ' || n
    from generate_series(500, 528) n;
  `);
  return pg;
};
...
describe("Unacademy NEET fifteenth-batch quality-review package", () => {
  ...
  it("pins the immutable hash and prepared-only handoff", () => {
    ...
    expect(readiness).toContain("29 -> 32");
  ...
  it("rehearses all three reviews atomically on a production-shaped database", async () => {
    ...
    expect(result.rows[0]).toEqual({
      ...
      teacher_links: 161,
      reviews: 32,
      titles: ["Alcohols, Phenols & Ethers", "Fluid Mechanics", "Kinematics 1D"],
      source_titles: [
        "NEET: Alcohols, Phenols & Ethers | Class 12 | Unacademy NEET | Anoop V.",
        "Fluid Mechanics -  Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Physics | Mahendra Singh",
        "Kinematics 1D -  Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Physics | Mahendra Singh",
      ],
      teachers: [36, 34, 34],
      missing: [[], [], []],
    });
    ...
    const result = await pg.query(`
      select
        ...
        (select array_agg(title_review_status order by id) from public.playlists
          where id in (423,424,425)) as statuses
    `);
    expect(result.rows[0]).toEqual({
      reviews: 29,
