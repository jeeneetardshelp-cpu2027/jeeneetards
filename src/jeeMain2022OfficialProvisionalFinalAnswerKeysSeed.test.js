...
const manifest = JSON.parse(readFileSync(
  "docs/study-materials/jee-main-2022-official-provisional-final-answer-keys-manifest.json",
  "utf8",
));
...
describe("JEE Main 2022 official provisional final answer-key package", () => {
  it("contains the two distinct official Paper 1 result-stage keys", () => {
    expect(manifest.resources).toHaveLength(2);
    expect(manifest.coverage).toContain("official provisional final answer keys");
    expect(manifest.exclusions).toContain("Challenge-stage provisional answer keys");
    expect(manifest.exclusions).toContain("PROVISIONAL FINAL KEY");
    ...
    for (const resource of manifest.resources) {
      expect(resource.title).toContain("JEE Main 2022");
      expect(resource.title).toContain("Provisional Final Answer Key");
      expect(resource.title).toContain("Paper 1 B.E./B.Tech");
      expect(resource.officialTitle).toContain("PROVISIONAL FINAL KEY");
      expect(resource.officialLandingUrl).toMatch(/^https:\/\/jeemain\.nta\.nic\.in\/document\//);
  ...
  it("keeps challenge drafts, Paper 2 keys and worked solutions out of the package", () => {
    ...
    expect(seed).toContain("not the earlier challenge-stage draft");
    ...
    try {
      ...
      const materials = await db.query(`
        select count(*)::integer as count
         ...
         where title like 'JEE Main 2022%Provisional Final Answer Key (Paper 1 B.E./B.Tech)'
      ...
      const scopes = await db.query(`
        select count(*)::integer as count
         ...
         where m.title like 'JEE Main 2022%Provisional Final Answer Key (Paper 1 B.E./B.Tech)'
           and g.slug = 'jee'
      ...
      `);
      expect(allOfficialKeys.rows[0].count).toBe(10);
    } finally {
      await db.close();
    }
  });
