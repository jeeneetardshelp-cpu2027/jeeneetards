...
const sqlPath = "docs/sql/unacademy_neet_eleventh_batch_quality_review_2026-08-06.sql";
const readinessPath = "docs/unacademy-neet-eleventh-batch-quality-readiness-2026-08-06.md";
const sql = readFileSync(sqlPath, "utf8");
const readiness = readFileSync(readinessPath, "utf8");
const expectedHash = "eae13bfeb83df0105da1c3fd4b87d3f9caa9330c86fec7fc428833e551ec82dc";
...
const seedProductionShape = async () => {
  ...
  await pg.exec(`
    ...
    insert into public.playlists values
      (414, 'NEET: Chemical Equilibrium - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Anoop Vashishtha', 'PLsgHooHkqhhPqS8MzgJCKn9bJwGRsR3Jl', 147, 2, 2,
       array['11th'], '11th'),
      (415, 'NEET: Surface Chemistry - Playlist | Class 12 | Unacademy NEET | Live Daily | NEET Chemistry | Anoop Vashishtha', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Anoop Vashishtha', 'PLsgHooHkqhhP5Nu98FZfS--EqYQpo15KT', 147, 2, 2,
       array['12th'], '12th'),
      (416, 'NEET: P Block Elements - Playlist | Class 12 | UnaPlaylist | Class 12 | Unacademy NEET | Live Daily | NEET Chemistry | Anoop Vashishtha', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Anoop Vashishtha', 'PLsgHooHkqhhM_8IsqTEL1V6sDYskLuymO', 147, 2, 2,
       array['12th'], '12th');

    insert into public.videos select n, 1 from generate_series(1, 4603) n;
    ...
    update public.chapters set name = 'Chemical Equilibrium', subject_id = 2 where id = 30;
    update public.chapters set name = 'Surface Chemistry', subject_id = 2 where id = 32;
    update public.chapters set name = 'P-Block Elements', subject_id = 2 where id = 93;
    ...
    insert into public.playlist_videos
    select n, 414, n, n - 1304 from generate_series(1305, 1314) n;
    insert into public.playlist_videos
    select n, 415, n, n - 1314 from generate_series(1315, 1319) n;
    insert into public.playlist_videos
    select n, 416, n, n - 1319 from generate_series(1320, 1329) n;
    insert into public.playlist_videos
    select n, 500, 1 + ((n - 1) % 4603), n - 1329
    ...
    insert into public.playlist_teachers
    select n, 1 + ((n - 500) % 31), 'instructor', 1 from generate_series(500, 566) n;
    insert into public.playlist_teachers values
      (414, 36, 'instructor', 1), (415, 36, 'instructor', 1),
      (416, 36, 'instructor', 1);
    insert into public.playlist_quality_reviews
      (playlist_id, before_state, after_state, note)
    select n, '{}'::jsonb, '{}'::jsonb, 'existing review ' || n
    from generate_series(500, 519) n;
  `);
  return pg;
};
...
describe("Unacademy NEET eleventh-batch quality-review package", () => {
  it("pins the owner decision, exact courses, and canonical review calls", () => {
    expect(sql).toContain("d8125eb3-7281-43da-bfd4-61acd655121f");
  ...
  it("pins the immutable hash and prepared-only handoff", () => {
    ...
    expect(readiness).toContain("20 -> 23");
  ...
  it("rehearses all three reviews atomically on a production-shaped database", async () => {
    ...
    expect(result.rows[0]).toEqual({
      ...
      teacher_links: 152,
      reviews: 23,
      titles: ["Chemical Equilibrium", "Surface Chemistry", "P-Block Elements"],
      source_titles: [
        "NEET: Chemical Equilibrium - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha",
        "NEET: Surface Chemistry - Playlist | Class 12 | Unacademy NEET | Live Daily | NEET Chemistry | Anoop Vashishtha",
        "NEET: P Block Elements - Playlist | Class 12 | UnaPlaylist | Class 12 | Unacademy NEET | Live Daily | NEET Chemistry | Anoop Vashishtha",
      ],
      teachers: [36, 36, 36],
      missing: [[], [], []],
    });
    ...
    const result = await pg.query(`
      select
        ...
        (select array_agg(title_review_status order by id) from public.playlists
          where id in (414,415,416)) as statuses
    `);
    expect(result.rows[0]).toEqual({
      reviews: 20,
