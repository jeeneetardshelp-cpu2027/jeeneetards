...
const sqlPath = "docs/sql/unacademy_neet_fourteenth_batch_quality_review_2026-08-06.sql";
const readinessPath = "docs/unacademy-neet-fourteenth-batch-quality-readiness-2026-08-06.md";
const sql = readFileSync(sqlPath, "utf8");
const readiness = readFileSync(readinessPath, "utf8");
const expectedHash = "20640b0bff00fda560c1563cd295ff9798da0528d63efc69b3983dadea8f5965";
...
const seedProductionShape = async () => {
  ...
  await pg.exec(`
    ...
    insert into public.playlists values
      (420, 'NEET: Friction - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Physics | Mahendra Singh', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Mahendra Singh', 'PLsgHooHkqhhM5-Ujy03Tn7YjofINdftRM', 147, 2, 1,
       array['11th'], '11th'),
      (421, 'NEET: The Unit of Life - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Pradeep Singh', 'PLsgHooHkqhhM6fzJQ3Vhv7s6iOglsVJw2', 147, 2, 4,
       array['11th'], '11th'),
      (422, 'Plant Anatomy - Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Biology | Pradeep Singh', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Pradeep Singh', 'PLsgHooHkqhhPkkXnKHj60aao7jClkwioE', 147, 2, 4,
       array['11th'], '11th');

    insert into public.videos select n, 1 from generate_series(1, 4655) n;
    ...
    update public.chapters set name = 'Friction', subject_id = 1 where id = 7;
    update public.chapters set name = 'Cell: The Unit of Life', subject_id = 4 where id = 107;
    update public.chapters set name = 'Anatomy of Flowering Plants', subject_id = 4 where id = 97;
    ...
    insert into public.playlist_teachers
    select n, 1 + ((n - 500) % 30), 'instructor', 1 from generate_series(500, 572) n;
    insert into public.playlist_teachers values
      (420, 34, 'instructor', 1), (421, 33, 'instructor', 1), (422, 33, 'instructor', 1);
    insert into public.playlist_quality_reviews
      (playlist_id, before_state, after_state, note)
    select n, '{}'::jsonb, '{}'::jsonb, 'existing review ' || n
    from generate_series(500, 525) n;
  `);
  return pg;
};
...
describe("Unacademy NEET fourteenth-batch quality-review package", () => {
  ...
  it("pins the immutable hash and prepared-only handoff", () => {
    ...
    expect(readiness).toContain("26 -> 29");
  ...
  it("rehearses all three reviews atomically on a production-shaped database", async () => {
    ...
    expect(result.rows[0]).toEqual({
      ...
      teacher_links: 158,
      reviews: 29,
      titles: ["Friction", "Cell: The Unit of Life", "Anatomy of Flowering Plants"],
      source_titles: [
        "NEET: Friction - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Physics | Mahendra Singh",
        "NEET: The Unit of Life - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh",
        "Plant Anatomy - Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Biology | Pradeep Singh",
      ],
      teachers: [34, 33, 33],
      missing: [[], [], []],
    });
    ...
    const result = await pg.query(`
      select
        ...
        (select array_agg(title_review_status order by id) from public.playlists
          where id in (420,421,422)) as statuses
    `);
    expect(result.rows[0]).toEqual({
      reviews: 26,
