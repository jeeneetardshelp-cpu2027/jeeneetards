...
const manifest = JSON.parse(readFileSync(
  "docs/study-materials/jee-main-2016-official-papers-manifest.json",
  "utf8",
));

describe("JEE Main 2016 official question-paper package", () => {
  it("records the complete visually verified Paper 1 set from the archived official host", () => {
    expect(manifest.officialArchiveUrl).toBe(
      "https://www.cbse.gov.in/cbsenew/annual-report/CBSE_Annual_Report_2016_17.pdf",
    );
    expect(manifest.officialQuestionPapersPageUrl).toBe(
      "https://web.archive.org/web/20160419204149id_/http://jeemain.nic.in:80/webinfo/QuestionPapers2016.htm",
    );
    expect(manifest.language).toBe(
      "English and Hindi (offline); English, Hindi and Gujarati (online)",
    );
    expect(manifest.papers).toHaveLength(6);
    expect(manifest.papers.map((paper) => paper.pageCount)).toEqual([
      40, 40, 40, 40, 46, 50,
    ]);
    expect(manifest.papers.reduce((sum, paper) => sum + paper.pageCount, 0)).toBe(256);
    expect(new Set(manifest.papers.map((paper) => paper.sourceUrl)).size).toBe(6);
    expect(manifest.papers.slice(0, 4).every((paper) => (
      paper.languages.join(",") === "English,Hindi"
    ))).toBe(true);
    expect(manifest.papers.slice(4).every((paper) => (
      paper.languages.join(",") === "English,Hindi,Gujarati"
    ))).toBe(true);
    for (const paper of manifest.papers) {
      ...
      expect(paper.byteLength).toBeGreaterThan(1_000_000);
      expect(seed).toContain(paper.title);
      expect(seed).toContain(paper.sourceUrl);
      expect(seed).toContain(`, ${paper.pageCount})`);
    }
    expect(seed).toContain("All 256 pages were hashed and visually checked");
  ...
  it("uses the supported Hinglish filter while accurately labelling the file languages", () => {
    expect(seed).toContain("'pdf', 'Hinglish', 2016");
    expect(seed).toContain("English and Hindi");
    expect(seed).toContain("English, Hindi and Gujarati");
    expect(seed).toContain("no bilingual or trilingual");
  });
