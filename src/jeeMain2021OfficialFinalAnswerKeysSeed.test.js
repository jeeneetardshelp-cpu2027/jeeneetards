...
const seed = readFileSync(
  "docs/sql/study_materials_jee_main_2021_official_final_answer_keys_seed_2026-08-31.sql",
  "utf8",
);
...
const existing2023FinalKeysSeed = readFileSync(
  "docs/sql/study_materials_jee_main_2023_official_final_answer_keys_seed_2026-08-27.sql",
  "utf8",
);
const existing2022FinalKeysSeed = readFileSync(
  "docs/sql/study_materials_jee_main_2022_official_provisional_final_answer_keys_seed_2026-08-31.sql",
  "utf8",
);
const manifest = JSON.parse(readFileSync(
  "docs/study-materials/jee-main-2021-official-final-answer-keys-manifest.json",
  "utf8",
));
...
describe("JEE Main 2021 official final answer-key package", () => {
  it("contains the four distinct official Paper 1 final answer keys", () => {
    ...
    expect(manifest.resources.map(({ examYear, session, pageCount }) => [examYear, session, pageCount])).toEqual([
      [2021, 1, 78], [2021, 2, 78], [2021, 3, 11], [2021, 4, 7],
    ]);
    expect(new Set(manifest.resources.map(({ sourceUrl }) => sourceUrl)).size).toBe(4);
    expect(new Set(manifest.resources.map(({ sha256 }) => sha256)).size).toBe(4);

    for (const resource of manifest.resources) {
      expect(resource.title).toContain("JEE Main 2021");
      expect(resource.title).toContain("Final Answer Key");
      expect(resource.title).toContain("Paper 1 B.E./B.Tech");
      expect(resource.officialTitle).toContain("FINAL ANSWER KEY");
      expect(resource.resultCompiledOn).toMatch(/^2021-\d{2}-\d{2}$/);
      expect(resource.sourceUrl).toMatch(
        /^https:\/\/www\.nta\.ac\.in\/Download\/Notice\/Notice_\d+\.pdf$/,
      );
  ...
  it("keeps provisional keys, Paper 2 keys and worked solutions out of the package", () => {
    expect(manifest.exclusions).toContain("Paper 2 B.Arch/B.Planning");
  ...
  it("rehearses twice against the study-material schema without drift", async () => {
    ...
    try {
      ...
      await db.exec(existing2023FinalKeysSeed);
      await db.exec(existing2022FinalKeysSeed);
      await db.exec(seed);
      await db.exec(seed);
      ...
      const materials = await db.query(`
        select count(*)::integer as count
         ...
         where title like 'JEE Main 2021%Final Answer Key (Paper 1 B.E./B.Tech)'
           and rights_status = 'official_source'
           and review_status = 'approved'
      `);
      expect(materials.rows[0].count).toBe(4);
      ...
      const scopes = await db.query(`
        select count(*)::integer as count
         ...
         where m.title like 'JEE Main 2021%Final Answer Key (Paper 1 B.E./B.Tech)'
           and g.slug = 'jee'
      ...
      `);
      expect(scopes.rows[0].count).toBe(4);

      const allOfficialKeys = await db.query(`
        select count(*)::integer as count
          from public.study_materials
         where title like 'JEE Main % Answer Key (Paper 1 B.E./B.Tech)'
      `);
      expect(allOfficialKeys.rows[0].count).toBe(14);
    } finally {
      await db.close();
    }
  });
