...
let afterFloor = {};      // control query -> rows, captured AFTER the deployed floor
let afterFloorAlias = {}; // alias -> rows, same boundary
...
beforeAll(async () => {
  ...
  // The state this migration inherits: the deployed floor, nothing more. Every
  // control query is snapshotted HERE, so the recall assertions below compare
  // against what production answers today rather than against the pre-floor
  // world.
  for (const q of CONTROL_QUERIES) afterFloor[q] = await search(q);
  for (const { alias } of SEED) afterFloorAlias[alias] = await search(alias);

  // Preflight and six synthetic laws run inside this exec. Getting past the
...
// ---------------------------------------------------------------------------
// THE RISK THIS MIGRATION CARRIES, ASSERTED.
//
// The q_long prefilter is a lossless superset of the rank tiers only while
// q_long is one of q_tokens: tier 4 requires every token as a substring, so a
// tier-4 row necessarily contains it. This migration can take the anchor from
// the ALIAS expansion instead, and an alias word is not one of the typed
// tokens -- so in principle it can exclude a row the ranker would have matched,
// silently and with no error.
//
// Every control query is therefore snapshotted immediately after the DEPLOYED
// floor and compared with what the same query returns afterwards. The
// comparison is by identity, not by count, so a swap of one row for another is
// caught as well as a loss.
// ---------------------------------------------------------------------------
const idsOf = (rows) => (rows ?? []).map((r) => `${r.group_key}:${r.entity_id}`).sort();

describe("the anchor never costs a row that answered before it", () => {
  it("returns a superset of the deployed floor's rows for every control query", async () => {
    for (const q of CONTROL_QUERIES) {
      const wasIds = idsOf(afterFloor[q]);
      const nowIds = idsOf(await search(q));
      const lost = wasIds.filter((id) => !nowIds.includes(id));
      expect(lost, `"${q}" lost ${lost.length} row(s) the deployed floor returned: ${lost.join(", ")}`).toEqual([]);
    }
  });

  it("returns a superset for every alias whose own words can carry the scan", async () => {
    // CASE 1 of the rule: when the typed content anchor clears the floor it
    // wins outright, so the needle is byte-identical to the deployed floor's
    // and the row set must be too. This is where "lossless" is a claim worth
    // asserting, and it covers all but a handful of aliases.
    for (const { alias } of SEED) {
      const typed = (await pg.query(
        "select public.search_floor_anchor(public.search_token_anchor(public.search_content_tokens((public.search_query_tokens($1)).q_tokens))) as a",
        [alias],
      )).rows[0].a;
      if (typed === null) continue; // case 2, asserted separately below
      const wasIds = idsOf(afterFloorAlias[alias]);
      const nowIds = idsOf(await search(alias));
      const lost = wasIds.filter((id) => !nowIds.includes(id));
      expect(lost, `alias "${alias}" anchors on its own word "${typed}" and still lost ${lost.join(", ")}`).toEqual([]);
    }
  });

  it("trades noise for the answer on the aliases whose own words cannot", async () => {
    // CASE 2: the typed side has nothing that can drive the index, so the
    // expansion carries it. The needle genuinely changes, and pretending the
    // row set is unchanged would be asserting a falsehood -- so assert the
    // trade instead.
    //
    // "p and c" is the whole reason this migration exists. Its tokens are
    // p/and/c; the deployed floor hands the index the stopword "and" and, on
    // PRODUCTION, that scans 1328 of 5533 titles and is cancelled -- 500 57014,
    // 0/3 runs. So the rows this fixture shows it "losing" are rows production
    // never returns. What it loses here is "Class 11 Physics Full Course",
    // which matched on the 1-character tokens and is exactly the noise
    // src/PlaylistBrowse.test.jsx:857 measured as "197 of ~500 courses, which
    ...
    for (const { alias, expansion } of SEED) {
      ...
      checked += 1;
      const titles = (await search(alias)).map((r) => r.title);
      expect(
        titles.some((t) => t.toLowerCase().includes(expansion.toLowerCase().split(" ").pop())),
        `alias "${alias}" fell to its expansion "${expansion}" and still did not reach it; got: ${titles.join(" | ")}`,
      ).toBe(true);
    }
    // If this ever reaches zero the test has stopped testing anything.
    expect(checked, "no alias exercised the expansion branch").toBeGreaterThan(0);
  });
});

describe("the anchor rule itself, through the deployed functions", () => {
  const anchorOf = async (typed, alias, fallback) => {
    const r = await pg.query("select public.search_anchor($1, $2, $3) as a", [typed, alias, fallback]);
    return r.rows[0].a;
  };

  it("lets a literal match win, so an alias can never displace one", async () => {
    expect(await anchorOf("shm", "harmonic", "shm")).toBe("shm");
  });

  it("falls to the LONGEST candidate when the typed side has nothing usable", async () => {
    // This is the whole fix for "p and c": the raw fallback is the stopword
    // "and", the expansion offers "combinations", and the longer one wins.
    expect(await anchorOf(null, "combinations", "and")).toBe("combinations");
  });

  it("returns null when nothing anywhere can drive the index", async () => {
    // Null is read by the body as "return no rows", which is what replaces a
    // three-second scan that ends in a cancelled statement.
    expect(await anchorOf(null, "ab", "cd")).toBeNull();
  });

  it("floors at three characters, in one place", async () => {
    const r = await pg.query("select public.search_min_anchor_len() as n");
    expect(r.rows[0].n).toBe(3);
    expect(await anchorOf("ac", null, "ac")).toBeNull();
  });
});
