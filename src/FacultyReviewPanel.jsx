...
export default function FacultyReviewPanel() {
  ...
  return (
    <section>
      ...
      {message && (
        // Success stays a literal emerald pair (no success token; fg + bg are
        // set together, so it reads in both themes). Failure uses the danger
        // tokens, which do follow the theme.
        <p className={`mt-4 rounded-xl border p-3 text-sm ${message.ok ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-danger-line bg-danger-soft text-danger"}`}>
          {message.text}
        </p>
      )}
      {error && (
        // Amber stays literal: no warning token, self-consistent fg + bg pair.
      ...
      {loading ? (
        <div className="mt-5 space-y-3" aria-busy="true">
          {[1, 2, 3].map((n) => <div key={n} className="h-20 animate-pulse rounded-xl bg-surface-2" />)}
        </div>
                  ...
                  <div className={`border-t ${t.divider} p-4`}>
                    ...
                    {requiresSplit ? (
                      <>
                        ...
                        <button
                          ...
                          className={`mt-3 inline-flex min-h-11 items-center gap-2 rounded-xl px-4 text-sm font-medium ${t.accentBg} ${t.accentInk} disabled:opacity-40`}
                        >
                          <Users className="h-4 w-4" /> Split into selected faculty
                        </button>
                      </>
                    ) : (
                      <div className={`rounded-xl ${t.raised} p-3`}>
                        <label className={`text-xs font-medium ${t.muted}`} htmlFor={`new-faculty-${group.normalized}`}>Create as a new person</label>
                        <div className="mt-2 flex flex-col gap-2 sm:flex-row">
                          ...
                          <button
                            ...
                            className={`min-h-11 rounded-xl px-4 text-sm font-medium ${t.accentBg} ${t.accentInk} disabled:opacity-40`}
                          >
                            Create separately
                          </button>
                        </div>
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      ...
                      <button
                        ...
                        className="inline-flex min-h-11 items-center gap-1 rounded-xl border border-danger-line px-4 text-sm text-danger"
                      >
