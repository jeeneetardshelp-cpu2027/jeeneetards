...
const sqlPath = "docs/sql/unacademy_neet_thirteenth_batch_quality_review_2026-08-06.sql";
const readinessPath = "docs/unacademy-neet-thirteenth-batch-quality-readiness-2026-08-06.md";
const sql = readFileSync(sqlPath, "utf8");
const readiness = readFileSync(readinessPath, "utf8");
const expectedHash = "6af3c714f816a6fd898a32ec3068d77059c285c054b7256089a928ce4aaeabb8";
...
const seedProductionShape = async () => {
  ...
  await pg.exec(`
    ...
    insert into public.playlists values
      (418, 'NEET: Thermodynamics - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Anoop Vashishtha', 'PLsgHooHkqhhMCPaz0b6MC-BhUeUgqhRFe', 147, 2, 2,
       array['11th'], '11th'),
      (419, 'Coordination Compounds - Playlist | Inorganic Chemistry | Class 12 | Unacademy NEET | LIVE DAILY | NEET Chemistry | Ashwani Tyagi', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Ashwani Tyagi', 'PLsgHooHkqhhMbdtmdvS2bUG_lYX9Ev43f', 147, 2, 2,
       array['12th'], '12th');

    insert into public.videos select n, 1 from generate_series(1, 4641) n;
    ...
    update public.chapters set name = 'Thermodynamics', subject_id = 2 where id = 36;
    update public.chapters set name = 'Coordination Compounds', subject_id = 2 where id = 87;
    ...
    insert into public.playlist_teachers
    select n, 1 + ((n - 500) % 30), 'instructor', 1 from generate_series(500, 570) n;
    insert into public.playlist_teachers values
      (418, 36, 'instructor', 1), (419, 32, 'instructor', 1);
    insert into public.playlist_quality_reviews
      (playlist_id, before_state, after_state, note)
    select n, '{}'::jsonb, '{}'::jsonb, 'existing review ' || n
    from generate_series(500, 523) n;
  `);
  return pg;
};
...
describe("Unacademy NEET thirteenth-batch quality-review package", () => {
  it("pins the exact courses and canonical review calls", () => {
    expect(sql.match(/v_result := public\.review_playlist_quality\(/g)).toHaveLength(2);
    for (const value of [
  ...
  it("allows only guarded source-title capture as a direct table write", () => {
    ...
    expect(sql).toContain("if v_updated <> 2 then");
  ...
  it("pins the immutable hash and prepared-only handoff", () => {
    ...
    expect(readiness).toContain("24 -> 26");
  ...
  it("rehearses both reviews atomically on a production-shaped database", async () => {
    ...
    expect(result.rows[0]).toEqual({
      ...
      teacher_links: 155,
      reviews: 26,
      titles: ["Thermodynamics", "Coordination Compounds"],
      source_titles: [
        "NEET: Thermodynamics - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha",
        "Coordination Compounds - Playlist | Inorganic Chemistry | Class 12 | Unacademy NEET | LIVE DAILY | NEET Chemistry | Ashwani Tyagi",
      ],
      teachers: [36, 32],
      missing: [[], []],
    });
    ...
    const result = await pg.query(`
      select
        ...
        (select array_agg(title_review_status order by id) from public.playlists
          where id in (418,419)) as statuses
    `);
    expect(result.rows[0]).toEqual({
      reviews: 24,
      source_titles: [null, null],
