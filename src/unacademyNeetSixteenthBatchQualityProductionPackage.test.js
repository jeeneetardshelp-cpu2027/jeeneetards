...
const sqlPath = "docs/sql/unacademy_neet_sixteenth_batch_quality_review_2026-08-07.sql";
const readinessPath = "docs/unacademy-neet-sixteenth-batch-quality-readiness-2026-08-07.md";
const sql = readFileSync(sqlPath, "utf8");
const readiness = readFileSync(readinessPath, "utf8");
const expectedHash = "55470c5d03f5f4fdd0763a47d31dd898b24b6895c134d6e7fbf33e4955eea00d";
...
const seedProductionShape = async () => {
  ...
  await pg.exec(`
    ...
    insert into public.playlists values
      (426, 'NEET: Applications Of Biotechnology | Live Daily 2.0 | Unacademy NEET | Seep Pahuja', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Seep Pahuja', 'PLsgHooHkqhhP1V_qdWDRNO0MczNtM6Q1m', 147, 2, 4,
       array['12th'], '12th'),
      (427, 'NEET: The Living World - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Dr Sachin Kapur | Pradeep Singh', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Dr. Sachin Kapur', 'PLsgHooHkqhhNWiiYtSlpdjPEVYhHqtCkR', 147, 2, 4,
       array['11th'], '11th'),
      (428, 'NEET: Reproductive Health - Playlist | Class 12 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Dr Sachin Kapur', null,
       false, 'pending', 'pending', 'full-course', 'hinglish', 'intermediate',
       'Dr. Sachin Kapur', 'PLsgHooHkqhhPZfPFIHshnh3J0Nsod2uDw', 147, 2, 4,
       array['12th'], '12th');

    insert into public.videos select n, 1 from generate_series(1, 4699) n;
    ...
    update public.chapters set name = 'Biotechnology and its Applications', subject_id = 4 where id = 102;
    update public.chapters set name = 'The Living World', subject_id = 4 where id = 127;
    update public.chapters set name = 'Reproductive Health', subject_id = 4 where id = 123;
    ...
    insert into public.teachers values
      (40, 'Seep Pahuja', 'seep pahuja', 'seep-pahuja', true),
      (41, 'Dr. Sachin Kapur', 'sachin kapur', 'sachin-kapur', true);
    ...
    insert into public.playlist_teachers
    select n, 1 + ((n - 500) % 32), 'instructor', 1 from generate_series(500, 578) n;
    insert into public.playlist_teachers values
      (426, 40, 'instructor', 1), (427, 41, 'instructor', 1), (428, 41, 'instructor', 1);
    insert into public.playlist_quality_reviews
      (playlist_id, before_state, after_state, note)
    select n, '{}'::jsonb, '{}'::jsonb, 'existing review ' || n
    from generate_series(500, 531) n;
  `);
  return pg;
};
...
describe("Unacademy NEET sixteenth-batch quality-review package", () => {
  it("pins the exact courses and canonical review calls", () => {
    ...
    for (const value of [
      "PLsgHooHkqhhP1V_qdWDRNO0MczNtM6Q1m",
      "PLsgHooHkqhhNWiiYtSlpdjPEVYhHqtCkR",
      "PLsgHooHkqhhPZfPFIHshnh3J0Nsod2uDw",
  ...
  it("allows only guarded source-title capture as a direct table write", () => {
    ...
    for (const fragment of [
      ...
      "count(*) from public.playlist_quality_reviews) <> 32",
  ...
  it("pins the immutable hash and successful production evidence", () => {
    ...
    expect(readiness).toContain("32 -> 35");
  ...
  it("rehearses all three reviews atomically on a production-shaped database", async () => {
    ...
    expect(result.rows[0]).toEqual({
      ...
      teacher_links: 164,
      reviews: 35,
      titles: ["Applications of Biotechnology", "The Living World", "Reproductive Health"],
      source_titles: [
        "NEET: Applications Of Biotechnology | Live Daily 2.0 | Unacademy NEET | Seep Pahuja",
        "NEET: The Living World - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Dr Sachin Kapur | Pradeep Singh",
        "NEET: Reproductive Health - Playlist | Class 12 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Dr Sachin Kapur",
      ],
      teachers: [40, 41, 41],
      missing: [[], [], []],
    });
    ...
    const result = await pg.query(`
      select
        ...
        (select array_agg(title_review_status order by id) from public.playlists
          where id in (426,427,428)) as statuses
    `);
    expect(result.rows[0]).toEqual({
      reviews: 32,
