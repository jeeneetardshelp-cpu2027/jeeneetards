...
const seed = readFileSync(
  "docs/sql/study_materials_jee_main_2014_papers_seed_2026-08-13.sql",
  "utf8",
);
const manifest = JSON.parse(readFileSync(
  "docs/study-materials/jee-main-2014-official-papers-manifest.json",
  "utf8",
));

describe("JEE Main 2014 official question-paper package", () => {
  it("records the complete visually verified Paper 1 set from the archived official host", () => {
    expect(manifest.officialQuestionPapersPageUrl).toBe(
      "https://web.archive.org/web/20150420171339id_/http://jeemain.nic.in:80/webinfo/QuestionPapers2014.htm",
    );
    expect(manifest.officialDisclosureUrl).toBe(
      "https://www.cbse.gov.in/attach/important_notice_jee_main_2014.pdf",
    );
    ...
    expect(manifest.papers.map((paper) => paper.pageCount)).toEqual([
      40, 40, 40, 40, 41, 48, 39, 48,
    ]);
    expect(manifest.papers.reduce((sum, paper) => sum + paper.pageCount, 0)).toBe(336);
    ...
    for (const paper of manifest.papers) {
      ...
      expect(paper.byteLength).toBeGreaterThan(400_000);
      expect(seed).toContain(paper.title);
      expect(seed).toContain(paper.sourceUrl);
      expect(seed).toContain(`, ${paper.pageCount})`);
    }
    expect(seed).toContain("All 336 pages were hashed and visually checked");
  ...
  it("uses the supported Hinglish filter while accurately labelling both file languages", () => {
    expect(seed).toContain("'pdf', 'Hinglish', 2014");
