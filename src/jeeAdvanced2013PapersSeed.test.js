...
describe("JEE Advanced 2013 question-paper seed", () => {
  it("contains both visually verified official bilingual PDFs and page counts", () => {
    for (const [file, pages] of expected) {
      expect(seed).toContain(`https://jeeadv.ac.in/past_qps/${file}`);
      expect(seed).toContain(`'Hinglish', ${pages}`);
    }
    expect(seed.match(/https:\/\/jeeadv\.ac\.in\/past_qps\/2013_[12]\.pdf/g)).toHaveLength(2);
    expect(seed).toContain("English + Hindi");
    expect(seed).toContain("Official bilingual");
    expect(seed).toContain("with answers");
    expect(seed).toContain("'previous_year_paper'");
    expect(seed).toContain("'official_source'");
    expect(seed).toContain("2013");
  });
