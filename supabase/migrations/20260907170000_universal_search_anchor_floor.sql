-- ============================================================================
-- RESTORED TO THE CHAIN 7 Sep 2026, unchanged below this banner. IT IS APPLIED.
--
-- e052d80 ("Revert the anchor floor: its premise did not survive measurement")
-- deleted this file, saying it "is unapplied, so this removes it from the chain
-- rather than reversing anything on production". The first clause is wrong, and
-- everything else followed from it. The remote migration history lists
-- 20260907170000, and the functions this file creates answer on production:
--
--     search_min_anchor_len()                      -> 3
--     search_floor_anchor(kinematics)            -> "kinematics"
--     search_content_tokens([ac, ka, matlab])      -> ["ac"]
--
-- So deleting the file removed the RECORD, not the change. Production kept the
-- anchor floor and the chain stopped describing it, which is the drift this
-- directory exists to end -- and it broke `db push` outright: the CLI refuses
-- to run with a remote version it cannot find locally, which is how this was
-- noticed at all.
--
-- The CLI suggests `migration repair --status reverted 20260907170000`. DO NOT.
-- That records this as never having run, which is false, and would hide five
-- live functions from everyone who reads this directory afterwards. The fix for
-- a missing applied migration is to land the file, exactly as on 2 Sep.
--
-- THIS IS NOT A JUDGEMENT ON THE REVERT. If the anchor floor should come out of
-- production, that needs a NEW migration that drops these functions and restores
-- universal_search -- deleting a file cannot do it. Restoring this one only
-- makes the chain describe the database again.
-- ============================================================================
-- ============================================================================
