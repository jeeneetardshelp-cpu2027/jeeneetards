-- ============================================================================
-- APPLIED 7 Sep 2026, AND ITS ORIGINAL RATIONALE WAS WRONG. Both are true, so
-- read this banner before the one below it.
--
-- WHAT IS TRUE: this is applied to production and it works. "p and c" answered
-- 500 57014 before it and answers 200 with 166 rows after, three runs out of
-- three, around 1.1 s. That is a curated shorthand this site ships and could
-- not answer.
--
-- WHAT IS NOT TRUE: the selectivity story below. The table of q_long corpus
-- frequencies was built from SINGLE-TOKEN queries, where the needle IS the
-- whole query -- so it measured query cost and attributed it to the needle.
-- Re-measured with this migration applied, three runs each:
--
--     "and"          200 x3, ~2.3 s, 7726 rows   (the table says 500)
--     "the and for"  200 x3, ~1.7 s,   50 rows
--     "tio"          200, 200, 500 57014, ~3.2 s <-- STILL FLIPS
--
-- So this is not the general fix its header claims. "tio" sits on the timeout
-- and flips with load, exactly as before. What this migration reliably fixes is
-- "p and c", and the honest reason is the one 20260907093000's corrected header
-- gives: the alias expansion adds a second set of predicates to every pillar,
-- and "p and c" is the only multi-token alias whose typed tokens are all one
-- character.
--
-- WHY IT IS BACK IN THE CHAIN. It was deleted on the belief that it was
-- unapplied. It was not -- it had been pushed about an hour earlier. Deleting
-- the file left production running a universal_search the repository could not
-- reproduce: "migration list" showed 20260907170000 remote with no local file,
-- and a from-scratch replay would have built the pre-anchor-floor body. That is
