...
| `20260907160000_browse_servable_floor_correction.sql` | **Staged, NOT applied.** Reduces `search_is_servable` to `length(q_long) >= 3`, dropping the token-count branch `20260907091500` shipped. That branch required one token of FOUR characters whenever a query had several, which refused `"ac the of"` (11 rows), `"ph the of"` (7) and `"ac kya hai"` (11) — Hinglish queries that answer today. The signature keeps its unused `p_tokens` argument so the two browse matchers need no re-emission. |
| `20260907170000_universal_search_anchor_floor.sql` | **Staged, NOT applied.** Adds five IMMUTABLE helpers (`search_min_anchor_len`, `search_floor_anchor`, `search_content_tokens`, `search_token_anchor`, `search_anchor`) and re-emits `universal_search` so the index prefilter scans on the typed content anchor when it clears three characters, else the longest floored alias or raw fallback, else nothing at all. Fixes the one acceptance query `20260907093000` sets itself and misses: `"p and c"` is a shorthand this site ships and answers 500 57014 on production, because the length floor's rescue hands the index the stopword `and` (1328 of 5533 titles). Deliberately NOT `search_is_servable`, the floor the browse RPCs use, and rejected twice for two different reasons: as shipped in `20260907091500` it branched on token count and refused `"ac the of"` (11 rows), `"ph the of"` (7) and `"ac kya hai"` (11), all of which answer today; since `20260907160000_browse_servable_floor_correction` it is `length(q_long) >= 3` and no longer over-refuses, but a length rule still cannot tell `the` (685 titles, answers) from `and` (1328, cancelled), so `"p and c"` clears it and times out anyway. The function body is the deployed text of `20260907093000` copied from the file with one statement inserted, so nothing is clobbered; the browse RPCs are untouched because they already have their own floor. Rehearsed in `src/universalSearchAnchorFloorSqlRehearsal.test.js` (18 tests), which snapshots every control query and alias after the deployed floor and asserts the anchor never loses a row — proven to catch loss: nulling `q_alias_long` passes all six of the migration's synthetic laws and still fails the recall assertion. |
