...
| `20260902170000_search_aliases.sql` | **Staged, not applied — and now the only pending file.** Teaches search the shorthand students say out loud (`shm`, `nlm`, `emi`, `ktg`, `moi`, `rot mech`, `pnc`, `aod`): ~39 seeded aliases in an admin-extendable table, plus `search_expand_aliases` / `search_rank_aliased`, and it re-emits `universal_search` to use them. Rehearsed on a real engine in `src/searchAliasesSqlRehearsal.test.js` (66 tests). Complementary to the client layer in `src/searchAliases.js`, whose 8 keys do not intersect these 32. |
...
| `docs/sql/universal_search_material_words_2026-09-02.sql` | **It collides with `search_aliases`.** Both re-emit `public.universal_search`, and this one contains no reference to `search_aliases`, `search_expand_aliases` or `search_rank_aliased`. Applied after the alias migration — which its later timestamp guarantees — it would silently revert aliasing: the table and helpers would survive, orphaned, while `universal_search` stopped calling them. Restoring it means rebuilding it **on top of** the alias definition, not merely re-timestamping it. Its own content is sound (a bare `notes` returning all 225 notes and sheets is the deliberate change it documents); the problem is the pair, not the file. |
