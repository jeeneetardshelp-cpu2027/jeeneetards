...
| `20260902164500_search_gap_log.sql` | **Staged, and deliberately HELD — do not push.** Creates `public.search_gap_log` (query text, script-neutral `query_key`, result count, timestamp), two indexes, RLS with an admin-only read policy, and the anonymous `log_search_gap` RPC, so a search that returns nothing is remembered instead of vanishing. It stores no user id, session, IP, fingerprint, referrer or filter state, and the RPC takes no identity parameter, so no row can be joined to a person. The hold is not about the design: it is a new server-side path carrying free text a student typed, and `src/PrivacyPolicy.jsx` does not name the table yet. Whether this log should exist at all is an owner decision recorded in `docs/legal_release_inputs.md`. Applying it first would collect data the policy says is not collected. `src/searchGapPrivacyContract.test.js` keeps the `DO NOT APPLY YET` banner in place until the disclosure lands; the site is unaffected meanwhile, because `src/searchGapLog.js` swallows the missing-function error and goes quiet. |
| `20260902170000_search_aliases.sql` | **Staged, not applied.** Teaches search the shorthand students say out loud: `SHM`, `PnC`, `AoD`, `EMI`, `rot mech`, `salt analysis` cleared no tier, because none of those letters appear in "Simple Harmonic Motion" or "Permutations and Combinations" and three characters cannot clear the word-similarity floor against a 30-character title. Adds `public.search_aliases` (public read of active rows, admin write) with an `updated_at` trigger, the `search_expand_aliases` and `search_rank_aliased` helpers, and re-emits `universal_search` to expand abbreviations inside a query as a second ranking pass. Complements rather than duplicates the client layer in `src/searchAliases.js`, which rewrites a whole query before the RPC runs: 8 client keys against 32 here, verified to have zero overlap, so neither can shadow the other. |
...
> **The whole pending queue is blocked, and that is on purpose.** Push takes
> every pending migration in timestamp order, and the held privacy file
> (`20260902164500`) sorts first, so there is no `db push` that applies the two
> search migrations behind it without also applying the search gap log. Nothing
> pending can ship until the Privacy Policy names `search_gap_log` and the owner
> decides that log should exist. Do not reach for `--include-all`, and do not
> renumber a file to jump the queue: the ordering is not what is stopping you,
> the undisclosed data collection is.
