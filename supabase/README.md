...
| `20260907170000_universal_search_anchor_floor.sql` | **Applied** 7 Sep 2026 — and **restored to this directory** on the same day after being deleted from it. `e052d80` ("Revert the anchor floor") removed the file saying it "is unapplied, so this removes it from the chain rather than reversing anything on production". The first clause is false and the rest followed from it: the remote history lists `20260907170000`, and the functions it creates answer on production — `search_min_anchor_len()` → 3, `search_floor_anchor(kinematics)` → `kinematics`, `search_content_tokens([ac,ka,matlab])` → `[ac]`. Deleting the file removed the RECORD, not the change, and it broke `db push` outright — the CLI refuses to run against a remote version it cannot find locally, which is the only reason this was caught. The CLI suggests `migration repair --status reverted`; that would record it as never having run, which is false, and hide five live functions. The fix for a missing applied migration is to land the file. **This says nothing about whether the revert was right.** If the anchor floor should leave production, that needs a NEW migration dropping those functions and restoring `universal_search`; a deleted file cannot do it. |
| `20260907200000_shared_tokeniser_rescue_floor.sql` | **Applied** 7 Sep 2026 — confirmed by `migration list` and by measuring both matchers. Moves the rescue floor from `20260907093000` (filter filler only when a survivor is still ≥ 3 characters) out of `universal_search` and into the shared helper `search_query_tokens()`, whose own comment says it exists so browse "tokenises identically to the homepage". Without it, `20260907140000` reduced `"ac ka matlab"` to `[ac]` and /browse answered **0** where /search answered 26. **The rows do not come from the raw tokens** — `search_rank_tokens` has only conjunction tiers, and no title holds `ac`+`ka`+`matlab`. They come from the ALIAS pass (`ac` → Alternating Current) matching on alias tokens, which the query only reaches once its needle survives; verified beforehand with `"ac one shot"` (needle `shot`, 29 rows) against `"ac notes"` (needle `ac`, 0 rows). After applying: `"ac ka matlab"` 29+4, `"ac kya hai"` 29+4, `"ac the of"` 48+4, all previously 0+0. `"ph kya hai"` is still 0 (no alias reaches a lecture title) — unchanged, not a regression. Nothing regressed: kinematics 172, iit jee 195, x ray 8, def int 176, s block 65, `"ac one shot"` 29, `"shm kya hai"` 376, and `ac`/`3d`/`p c`/`a b c`/`p n c` all still refuse. **The one cost, paid and measured:** `"p and c"` went from 200/0 rows to **500 57014, 3/3, ~3.3s**, because the rescue keeps `[p,and,c]` and `and` clears the floor. I had guessed these matchers would come in under the ceiling since they cost about half of `universal_search` for the same needle; with the alias expansion on top they do not. Kept deliberately — `universal_search` has answered 500 for that query since `20260907093000`, so the two surfaces now agree, and `isServableQuery` refuses it client-side on both. |
