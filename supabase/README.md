...
| `20260904120000_profiles_only_username_is_public.sql` | **Applied** 4 Sep 2026 — confirmed by `migration list`, local and remote both at `20260904120000`. Closes a leak: signed out, with nothing but the public anon key, `profiles` returned every student's real name and Google profile photo. Only the claimed forum username is browser-readable now. `npm run verify:rls-boundary` passes 21/21 against production, and the boundary check covers the neighbouring tables. |
...
| `20260907093000_universal_search_q_long_floor.sql` | **Applied** 7 Sep 2026 — confirmed by `migration list`, local and remote both at `20260907093000`. Puts a length floor under `q_long` so filler removal cannot leave the index prefilter a needle too short to use. **Its own acceptance criteria are only two-thirds met.** Measured on production 7 Sep after applying: `"ac the of"` 200 (3/3) and `"ph the of"` 200 (3/3) as the file predicts, but `"p and c"` is still **500 57014, 0/3, ~3.3s** where the header says 200. The mechanism is that the floor's rescue falls back to the RAW token list, whose longest member here is the stopword `and` — 1328 of 5533 video titles, too common for the GIN index. The floor is a length rule; this failure is selectivity. No student hits it: `isServableQuery` refuses `p and c` (tokens 1/3/1) client-side on all three surfaces. |
