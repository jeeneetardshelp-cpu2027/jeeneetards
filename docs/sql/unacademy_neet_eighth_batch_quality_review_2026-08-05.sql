...
-- production courses 405-407 only. No catalogue rows are created or deleted.
...
begin
  if exists (select 1 from public.app_environment) then
    raise exception 'refusing Unacademy eighth-batch quality review: target is not production-empty';
  end if;
  ...
  if to_regprocedure(
    ...
    raise exception 'refusing Unacademy eighth-batch quality review: canonical v10 review contract is absent';
  end if;
  ...
  if public.content_quality_capability() @> jsonb_build_object(
    ...
    raise exception 'refusing Unacademy eighth-batch quality review: capability differs';
  end if;
  ...
  if (select count(*) from public.playlists) <> 388
    ...
    raise exception 'refusing Unacademy eighth-batch quality review: exact baseline differs';
  end if;
  ...
  if exists (
    ...
    raise exception 'refusing Unacademy eighth-batch quality review: reviewed course evidence differs';
  end if;
  ...
  if exists (
    ...
    raise exception 'refusing Unacademy eighth-batch quality review: verified teacher identities differ';
  end if;

  if exists (
    select 1
    from (values
      (405::bigint, 36::bigint),
      (406::bigint, 33::bigint),
      (407::bigint, 33::bigint)
  ...
  or (select count(*) from public.playlist_teachers
       where playlist_id in (405, 406, 407)) <> 3 then
    raise exception 'refusing Unacademy eighth-batch quality review: course-teacher links differ';
  end if;

  if exists (
    select 1 from public.playlist_quality_reviews
     where playlist_id in (405, 406, 407)
  ) then
    raise exception 'refusing Unacademy eighth-batch quality review: target already reviewed';
  end if;
  ...
  or not exists (select 1 from public.class_levels where id = 3 and slug = 'class-12') then
    raise exception 'refusing Unacademy eighth-batch quality review: reference data differs';
  end if;
  ...
  if v_protected.protected_courses <> 82
    ...
    raise exception 'refusing Unacademy eighth-batch quality review: protected JEE mismatch (%)',
...
begin
  update public.playlists p
    ...
    from (values
      (405::bigint, 'Redox Reactions | Class 11 | Unacademy NEET | Anoop Vashishtha'::text),
      (406::bigint, 'NEET: Cell Organelles Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | Pradeep Singh'::text),
      (407::bigint, 'NEET: Molecular Basis of Inheritance - Playlist | Class 12 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh'::text)
  ...
  if v_updated <> 3 then
    raise exception 'Unacademy eighth-batch quality review source-title capture mismatch (%)', v_updated;
  end if;
end
...
declare
  ...
  v_note constant text :=
    'Owner-reviewed official-channel attribution decision 809b153c-b5ff-48e0-a869-02faa49b0e8f; Unacademy NEET eighth-batch quality transition prepared 2026-08-05.';
begin
  v_result := public.review_playlist_quality(405, 'Redox Reactions', array[36]::bigint[],
  ...
  if (v_result->>'quality_ready')::boolean is distinct from true
    ...
    raise exception 'course 405 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(406, 'Cell Organelles', array[33]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_note);
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 406 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(407, 'Molecular Basis of Inheritance', array[33]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_note);
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 407 did not become quality-ready (%)', v_result;
  end if;
end
$review$;
...
begin
  if (select count(*) from public.playlists) <> 388
    ...
    raise exception 'Unacademy eighth-batch quality review postflight total mismatch';
  end if;

  if exists (
    select 1
    from (values
      (405::bigint, 'Redox Reactions'::text,
       'Redox Reactions | Class 11 | Unacademy NEET | Anoop Vashishtha'::text, 36::bigint),
      (406::bigint, 'Cell Organelles'::text,
       'NEET: Cell Organelles Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | Pradeep Singh'::text, 33::bigint),
      (407::bigint, 'Molecular Basis of Inheritance'::text,
       'NEET: Molecular Basis of Inheritance - Playlist | Class 12 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh'::text, 33::bigint)
    ...
    raise exception 'Unacademy eighth-batch quality review course postflight mismatch';
  end if;
  ...
  if exists (
    ...
    from (values
      (405::bigint, 'Redox Reactions | Class 11 | Unacademy NEET | Anoop Vashishtha'::text,
       'Redox Reactions'::text, 36::bigint),
      (406::bigint, 'NEET: Cell Organelles Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | Pradeep Singh'::text,
       'Cell Organelles'::text, 33::bigint),
      (407::bigint, 'NEET: Molecular Basis of Inheritance - Playlist | Class 12 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh'::text,
       'Molecular Basis of Inheritance'::text, 33::bigint)
  ...
  or exists (
    ...
    raise exception 'Unacademy eighth-batch quality review audit postflight mismatch';
  end if;
  ...
  if v_protected.protected_courses <> 82
     or v_protected.protected_memberships <> 1304
     or v_protected.protected_fingerprint <> '30eee4a4a6842e5beeb7c97083d7f812' then
    raise exception 'Unacademy eighth-batch quality review changed protected JEE (%)',
      row_to_json(v_protected);
  end if;
end
$postflight$;
