-- Official IIT-JEE 2008 question papers with answers in English, preserved in
...
begin
  ...
  if jee_id is null then
    raise exception 'IIT-JEE 2008 PAPERS PREFLIGHT: JEE learning goal is missing';
  end if;

  for resource in
    select * from (values
      ('IIT-JEE 2008 Paper 1 (English)',
       'Official IIT-JEE 2008 Paper 1 question paper with answers in English, covering Chemistry, Mathematics and Physics.',
       'https://jeeadv.ac.in/past_qps/2008_1.pdf', 'English', 24),
      ('IIT-JEE 2008 Paper 2 (English)',
       'Official IIT-JEE 2008 Paper 2 question paper with answers in English, covering Chemistry, Mathematics and Physics.',
       'https://jeeadv.ac.in/past_qps/2008_2.pdf', 'English', 28)
  ...
  loop
    insert into public.study_materials (
      ...
      2008, resource.page_count, true, 'official_source',
  ...
  select count(*)::integer into batch_material_count
   ...
   where source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2008_[12][.]pdf$'
     and exam_year = 2008;
  if batch_material_count <> 2 then
    raise exception 'IIT-JEE 2008 PAPERS POSTFLIGHT: expected 2 materials, found %', batch_material_count;
  end if;
  ...
  select count(*)::integer into batch_scope_count
   ...
   where m.source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2008_[12][.]pdf$'
     and m.exam_year = 2008 and s.learning_goal_id = jee_id
  ...
  if batch_scope_count <> 2 then
    raise exception 'IIT-JEE 2008 PAPERS POSTFLIGHT: expected 2 JEE-only scopes, found %', batch_scope_count;
  end if;
end $$;
