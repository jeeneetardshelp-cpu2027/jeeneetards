-- Official JEE (Advanced) 2019 question papers in English and Hindi.
-- Every PDF stays on jeeadv.ac.in; JEENEETARD stores links and metadata only.
-- Whole-exam papers receive one JEE goal scope and no false class, subject,
-- chapter or lecture attachment. Safe to rerun.
...
begin
  ...
  if jee_id is null then
    raise exception 'JEE ADVANCED 2019 PAPERS PREFLIGHT: JEE learning goal is missing';
  end if;

  for resource in
    select * from (values
      ('JEE Advanced 2019 Paper 1 (English)',
       'Official JEE (Advanced) 2019 Paper 1 question paper in English, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2019_1_English.pdf', 'English', 28),
      ('JEE Advanced 2019 Paper 1 (Hindi)',
       'Official JEE (Advanced) 2019 Paper 1 question paper in Hindi, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2019_1_Hindi.pdf', 'Hindi', 30),
      ('JEE Advanced 2019 Paper 2 (English)',
       'Official JEE (Advanced) 2019 Paper 2 question paper in English, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2019_2_English.pdf', 'English', 31),
      ('JEE Advanced 2019 Paper 2 (Hindi)',
       'Official JEE (Advanced) 2019 Paper 2 question paper in Hindi, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2019_2_Hindi.pdf', 'Hindi', 33)
  ...
  loop
    insert into public.study_materials (
      ...
      'JEE (Advanced)', resource.source_url, 'pdf', resource.language, 2019,
  ...
  select count(*)::integer into batch_material_count
   ...
   where source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2019_[12]_(English|Hindi)[.]pdf$'
     and exam_year = 2019;
  if batch_material_count <> 4 then
    raise exception 'JEE ADVANCED 2019 PAPERS POSTFLIGHT: expected 4 materials, found %', batch_material_count;
  end if;
  ...
  select count(*)::integer into batch_scope_count
   ...
   where m.source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2019_[12]_(English|Hindi)[.]pdf$'
     and m.exam_year = 2019 and s.learning_goal_id = jee_id
  ...
  if batch_scope_count <> 4 then
    raise exception 'JEE ADVANCED 2019 PAPERS POSTFLIGHT: expected 4 JEE-only scopes, found %', batch_scope_count;
  end if;
end $$;
