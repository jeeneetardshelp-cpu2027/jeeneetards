-- Official JEE (Advanced) 2021 question papers in English and Hindi.
...
begin
  ...
  if jee_id is null then
    raise exception 'JEE ADVANCED 2021 PAPERS PREFLIGHT: JEE learning goal is missing';
  end if;

  for resource in
    select * from (values
      ('JEE Advanced 2021 Paper 1 (English)',
       'Official JEE (Advanced) 2021 Paper 1 question paper in English, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2021_1_English.pdf', 'English', 29),
      ('JEE Advanced 2021 Paper 1 (Hindi)',
       'Official JEE (Advanced) 2021 Paper 1 question paper in Hindi, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2021_1_Hindi.pdf', 'Hindi', 33),
      ('JEE Advanced 2021 Paper 2 (English)',
       'Official JEE (Advanced) 2021 Paper 2 question paper in English, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2021_2_English.pdf', 'English', 29),
      ('JEE Advanced 2021 Paper 2 (Hindi)',
       'Official JEE (Advanced) 2021 Paper 2 question paper in Hindi, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2021_2_Hindi.pdf', 'Hindi', 34)
  ...
  loop
    ...
    if not exists (
      ...
      insert into public.study_material_scopes (
        material_id, learning_goal_id
      ) values (
        target_material_id, jee_id
      );
    end if;
  end loop;
  ...
  select count(*)::integer into batch_material_count
   ...
   where source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2021_[12]_(English|Hindi)[.]pdf$'
     and exam_year = 2021;
  if batch_material_count <> 4 then
    raise exception 'JEE ADVANCED 2021 PAPERS POSTFLIGHT: expected 4 materials, found %', batch_material_count;
  end if;
  ...
  select count(*)::integer into batch_scope_count
   ...
   where m.source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2021_[12]_(English|Hindi)[.]pdf$'
     and m.exam_year = 2021
  ...
  if batch_scope_count <> 4 then
    raise exception 'JEE ADVANCED 2021 PAPERS POSTFLIGHT: expected 4 JEE-only scopes, found %', batch_scope_count;
  end if;
end $$;
