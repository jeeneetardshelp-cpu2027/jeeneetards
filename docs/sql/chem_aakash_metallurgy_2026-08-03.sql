...
--
-- Source     : https://www.youtube.com/playlist?list=PL7AAT-ai0VD5pEdkq8yZPgSmTyXKiflkF
-- Lessons    : 4 (playlist declares 4)
-- Chapters   : 55 Metallurgy
...
begin
  ...
  if exists (select 1 from public.playlists where title = 'Metallurgy — General Principles of Isolation of Elements') then
    raise exception 'course "%" already exists - this file has already been run', 'Metallurgy — General Principles of Isolation of Elements';
  end if;

  if exists (
    select 1 from public.videos
    where youtube_video_id = any(array[
      'q6_Ca8Q7wTQ', '4zthIKHjNnE', 'OylM5IjhEdM', 'r0PUMMEOhkQ'
    ])
  ) then
  ...
  insert into public.playlists (
    ...
    'Metallurgy — General Principles of Isolation of Elements',
    'Metallurgy — General Principles of Isolation of Elements',
    'Metallurgy taught in sequence, from the official Aakash NEET channel.',
  ...
  for v_row in
    select * from (values
      (1, 'q6_Ca8Q7wTQ', 'Concentration of Ores (Magnetic Sep, Froth Floatation) (Part 1)', 'Concentration of Ores (Magnetic Sep, Froth Floatation) | L1 | Metallurgy | Chem | Ashish Shekhar', 3296, 55),
      (2, '4zthIKHjNnE', 'Leaching, Roasting, Ellingham Diagram (Reduction to Metal) (Part 2)', 'Leaching, Roasting, Ellingham Diagram (Reduction to Metal) | L2 | Metallurgy | Chem | Ashish Shekhar', 3102, 55),
      (3, 'OylM5IjhEdM', 'Iron, Copper, Zinc Extraction (Part 3)', 'Iron, Copper, Zinc Extraction | L3 | Metallurgy | Chem | Ashish Shekhar', 3633, 55),
      (4, 'r0PUMMEOhkQ', 'Aluminium Extraction, Refining Techniques (Part 4)', 'Aluminium Extraction, Refining Techniques | L4 | Metallurgy | Chemistry | Ashish Shekhar', 3466, 55)
  ...
  if v_inserted <> 4 then
    raise exception 'expected 4 lessons for "%", inserted %', 'Metallurgy — General Principles of Isolation of Elements', v_inserted;
  end if;
  ...
  if exists (
    ...
    raise exception 'a lesson in "%" was left without a goal or class level', 'Metallurgy — General Principles of Isolation of Elements';
  end if;
end $$;
