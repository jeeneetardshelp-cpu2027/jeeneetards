...
--
-- Source     : https://www.youtube.com/playlist?list=PL7AAT-ai0VD7luKXJHL16AuMqJl-KOAFa
-- Lessons    : 1
-- Chapters   : 40 Chemistry in Everyday Life
...
begin
  ...
  if exists (select 1 from public.playlists where title = 'Chemistry in Everyday Life') then
    raise exception 'course "%" already exists - this file has already been run', 'Chemistry in Everyday Life';
  end if;
  ...
  insert into public.playlists (
    ...
    'Chemistry in Everyday Life',
    'Chemistry in Everyday Life, from the official Aakash NEET channel.',
  ...
  for v_row in
    select * from (values
      (1, 'M9a-kuKjTME', 'Chemistry in Everyday Life', 'Chemistry in Everyday Life Class 12 Chapter Explained | NEET 2023 Exam Preparation', 2019, 40)
  ...
  if v_inserted <> 1 then
    raise exception 'expected 1 lessons for "%", inserted %', 'Chemistry in Everyday Life', v_inserted;
  end if;
  ...
  if exists (
    ...
    raise exception 'a lesson in "%" was left without a goal or class level', 'Chemistry in Everyday Life';
  end if;
end $$;
