-- One verified official CBSE JEE Main 2017 Paper 1 answer-key PDF. It covers
-- the 2 April offline Sets A-D and the 8 and 9 April online examinations. The
-- preserved official homepage labels it "JEE (Main) 2017 Paper -1 Answer Key".
-- The PDF is dated 25 April 2017, after the published 18-22 April challenge
-- window, but the source does not explicitly call it a final answer key. Paper
-- 2 keys, candidate OMR sheets, question papers and third-party solutions are
-- excluded. This is an answer key only and contains no worked solutions.
-- JEENEETARD stores the preserved official link and catalogue metadata only;
-- the PDF is not mirrored. This whole-exam resource receives one JEE scope and
-- no false class, subject, chapter or lecture attachment. Safe to rerun;
-- collision, metadata and scope guards fail closed.

begin;

create temporary table jee_main_2017_answer_key_seed (
  title text not null,
...
insert into jee_main_2017_answer_key_seed (
  title, description, source_url, exam_year, page_count
) values (
  'JEE Main 2017 Answer Key (Paper 1 B.E./B.Tech)',
  'Official CBSE answer key for JEE Main 2017 Paper 1 B.E./B.Tech. The six-page PDF covers the 2 April offline Sets A-D and the 8 and 9 April online examinations. It is dated 25 April 2017, after the published answer-key challenge window. This is an answer key only and does not include worked solutions.',
  'https://web.archive.org/web/20170430233022id_/http://jeemain.nic.in:80/WebInfo/Handler/FileHandler.ashx?i=File&ii=78&iii=Y',
  2017,
  6
);
...
begin
  ...
  if jee_id is null then
    raise exception 'JEE MAIN 2017 ANSWER KEY PREFLIGHT: JEE learning goal is missing';
  end if;

  select count(*)::integer into source_collision_count
    from public.study_materials m
    join jee_main_2017_answer_key_seed r on r.source_url = m.source_url
   where m.title is distinct from r.title;
  if source_collision_count <> 0 then
    raise exception 'JEE MAIN 2017 ANSWER KEY PREFLIGHT: % source URL collisions', source_collision_count;
  end if;

  select count(*)::integer into title_collision_count
    from public.study_materials m
    join jee_main_2017_answer_key_seed r on r.title = m.title
   where m.source_url is distinct from r.source_url;
  if title_collision_count <> 0 then
    raise exception 'JEE MAIN 2017 ANSWER KEY PREFLIGHT: % title collisions', title_collision_count;
  end if;

  for resource in
    select * from jee_main_2017_answer_key_seed
  loop
    insert into public.study_materials (
      ...
      'Official JEE Main 2017 answer-key PDF linked through a preserved capture of the original jeemain.nic.in host. Linked only; not mirrored or redistributed by JEENEETARD.',
  ...
  select count(*)::integer into batch_material_count
    from public.study_materials m
    join jee_main_2017_answer_key_seed r
      on r.title = m.title and r.source_url = m.source_url;
  if batch_material_count <> 1 then
    raise exception 'JEE MAIN 2017 ANSWER KEY POSTFLIGHT: expected 1 material, found %', batch_material_count;
  end if;

  select count(*)::integer into metadata_mismatch_count
    from jee_main_2017_answer_key_seed r
  ...
  if metadata_mismatch_count <> 0 then
    raise exception 'JEE MAIN 2017 ANSWER KEY POSTFLIGHT: % metadata mismatches', metadata_mismatch_count;
  end if;
  ...
  if batch_scope_count <> 1 then
    raise exception 'JEE MAIN 2017 ANSWER KEY POSTFLIGHT: expected exactly 1 total scope, found %', batch_scope_count;
  end if;
  ...
  if batch_scope_count <> 1 then
    raise exception 'JEE MAIN 2017 ANSWER KEY POSTFLIGHT: expected 1 JEE-only scope, found %', batch_scope_count;
  end if;
end $$;
