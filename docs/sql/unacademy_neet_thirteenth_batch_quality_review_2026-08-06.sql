...
-- production courses 418-419 only. No catalogue rows are created or deleted.
...
begin
  if exists (select 1 from public.app_environment) then
    raise exception 'refusing Unacademy thirteenth-batch quality review: target is not production-empty';
  end if;
  ...
  if to_regprocedure(
    ...
    raise exception 'refusing Unacademy thirteenth-batch quality review: canonical v10 review contract is absent';
  end if;
  ...
  if public.content_quality_capability() @> jsonb_build_object(
    ...
    raise exception 'refusing Unacademy thirteenth-batch quality review: capability differs';
  end if;
  ...
  if (select count(*) from public.playlists) <> 400
    ...
    raise exception 'refusing Unacademy thirteenth-batch quality review: exact baseline differs';
  end if;
  ...
  if exists (
    ...
    from (values
      (
        418::bigint,
        'NEET: Thermodynamics - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha'::text,
        'Anoop Vashishtha'::text, 'PLsgHooHkqhhMCPaz0b6MC-BhUeUgqhRFe'::text,
        36::bigint, 12::bigint, 36::bigint, '11th'::text, 'class-11'::text
      ),
      (
        419::bigint,
        'Coordination Compounds - Playlist | Inorganic Chemistry | Class 12 | Unacademy NEET | LIVE DAILY | NEET Chemistry | Ashwani Tyagi'::text,
        'Ashwani Tyagi'::text, 'PLsgHooHkqhhMbdtmdvS2bUG_lYX9Ev43f'::text,
        87::bigint, 12::bigint, 32::bigint, '12th'::text, 'class-12'::text
      )
    ) expected(
      playlist_id, title, legacy_teacher, youtube_playlist_id, chapter_id,
      membership_count, teacher_id, class_label, class_slug
    )
    ...
    raise exception 'refusing Unacademy thirteenth-batch quality review: reviewed course evidence differs';
  end if;
  ...
  if exists (
    ...
    raise exception 'refusing Unacademy thirteenth-batch quality review: verified teacher identities differ';
  end if;

  if exists (
    select 1
    from (values
      (418::bigint, 36::bigint),
      (419::bigint, 32::bigint)
  ...
  or (select count(*) from public.playlist_teachers
       where playlist_id in (418, 419)) <> 2 then
    raise exception 'refusing Unacademy thirteenth-batch quality review: course-teacher links differ';
  end if;

  if exists (
    select 1 from public.playlist_quality_reviews
     where playlist_id in (418, 419)
  ) then
    raise exception 'refusing Unacademy thirteenth-batch quality review: target already reviewed';
  end if;
  ...
  or not exists (select 1 from public.chapters where id = 36 and name = 'Thermodynamics' and subject_id = 2)
  or not exists (select 1 from public.chapters where id = 87 and name = 'Coordination Compounds' and subject_id = 2) then
    raise exception 'refusing Unacademy thirteenth-batch quality review: reference data differs';
  end if;
  ...
  if v_protected.protected_courses <> 82
    ...
    raise exception 'refusing Unacademy thirteenth-batch quality review: protected JEE mismatch (%)',
...
begin
  update public.playlists p
    ...
    from (values
      (418::bigint, 'NEET: Thermodynamics - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha'::text),
      (419::bigint, 'Coordination Compounds - Playlist | Inorganic Chemistry | Class 12 | Unacademy NEET | LIVE DAILY | NEET Chemistry | Ashwani Tyagi'::text)
  ...
  if v_updated <> 2 then
    raise exception 'Unacademy thirteenth-batch quality review source-title capture mismatch (%)', v_updated;
  end if;
end
...
declare
  ...
  v_note constant text :=
    'Owner-reviewed Unacademy NEET thirteenth-batch faculty evidence; quality transition prepared 2026-08-06.';
begin
  v_result := public.review_playlist_quality(
    418, 'Thermodynamics', array[36]::bigint[],
  ...
  if (v_result->>'quality_ready')::boolean is distinct from true
    ...
    raise exception 'course 418 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(
    419, 'Coordination Compounds', array[32]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_note
  );
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 419 did not become quality-ready (%)', v_result;
  end if;
end
$review$;
...
begin
  if (select count(*) from public.playlists) <> 400
    ...
    raise exception 'Unacademy thirteenth-batch quality review postflight total mismatch';
  end if;

  if exists (
    select 1
    from (values
      (418::bigint, 'Thermodynamics'::text,
       'NEET: Thermodynamics - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha'::text, 36::bigint),
      (419::bigint, 'Coordination Compounds'::text,
       'Coordination Compounds - Playlist | Inorganic Chemistry | Class 12 | Unacademy NEET | LIVE DAILY | NEET Chemistry | Ashwani Tyagi'::text, 32::bigint)
    ...
    raise exception 'Unacademy thirteenth-batch quality review course postflight mismatch';
  end if;
  ...
  if exists (
    ...
    from (values
      (418::bigint,
       'NEET: Thermodynamics - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Chemistry | Anoop Vashishtha'::text,
       'Thermodynamics'::text, 36::bigint),
      (419::bigint,
       'Coordination Compounds - Playlist | Inorganic Chemistry | Class 12 | Unacademy NEET | LIVE DAILY | NEET Chemistry | Ashwani Tyagi'::text,
       'Coordination Compounds'::text, 32::bigint)
  ...
  or exists (
    ...
    raise exception 'Unacademy thirteenth-batch quality review audit postflight mismatch';
  end if;
  ...
  if v_protected.protected_courses <> 82
     or v_protected.protected_memberships <> 1304
     or v_protected.protected_fingerprint <> '30eee4a4a6842e5beeb7c97083d7f812' then
    raise exception 'Unacademy thirteenth-batch quality review protected JEE mismatch (%)',
      row_to_json(v_protected);
  end if;
end
$postflight$;
