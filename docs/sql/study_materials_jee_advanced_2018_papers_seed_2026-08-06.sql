-- Official JEE (Advanced) 2018 question papers in English.
...
begin
  ...
  if jee_id is null then
    raise exception 'JEE ADVANCED 2018 PAPERS PREFLIGHT: JEE learning goal is missing';
  end if;

  for resource in
    select * from (values
      ('JEE Advanced 2018 Paper 1 (English)',
       'Official JEE (Advanced) 2018 Paper 1 question paper in English, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2018_1.pdf', 'English', 32),
      ('JEE Advanced 2018 Paper 2 (English)',
       'Official JEE (Advanced) 2018 Paper 2 question paper in English, covering Mathematics, Physics and Chemistry.',
       'https://jeeadv.ac.in/past_qps/2018_2.pdf', 'English', 33)
  ...
  loop
    insert into public.study_materials (
      ...
      'JEE (Advanced)', resource.source_url, 'pdf', resource.language, 2018,
  ...
  select count(*)::integer into batch_material_count
   ...
   where source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2018_[12][.]pdf$'
     and exam_year = 2018;
  if batch_material_count <> 2 then
    raise exception 'JEE ADVANCED 2018 PAPERS POSTFLIGHT: expected 2 materials, found %', batch_material_count;
  end if;
  ...
  select count(*)::integer into batch_scope_count
   ...
   where m.source_url ~ '^https://jeeadv[.]ac[.]in/past_qps/2018_[12][.]pdf$'
     and m.exam_year = 2018 and s.learning_goal_id = jee_id
  ...
  if batch_scope_count <> 2 then
    raise exception 'JEE ADVANCED 2018 PAPERS POSTFLIGHT: expected 2 JEE-only scopes, found %', batch_scope_count;
  end if;
end $$;
