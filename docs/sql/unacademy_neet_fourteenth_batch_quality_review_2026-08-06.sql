...
-- production courses 420-422 only. No catalogue rows are created or deleted.
...
begin
  if exists (select 1 from public.app_environment) then
    raise exception 'refusing Unacademy fourteenth-batch quality review: target is not production-empty';
  end if;
  ...
  if to_regprocedure(
    ...
    raise exception 'refusing Unacademy fourteenth-batch quality review: canonical v10 review contract is absent';
  end if;
  ...
  if public.content_quality_capability() @> jsonb_build_object(
    ...
    raise exception 'refusing Unacademy fourteenth-batch quality review: capability differs';
  end if;
  ...
  if (select count(*) from public.playlists) <> 403
    ...
    raise exception 'refusing Unacademy fourteenth-batch quality review: exact baseline differs';
  end if;
  ...
  if exists (
    ...
    from (values
      (
        ...
        1::bigint, 7::bigint, 4::bigint, 34::bigint, '11th'::text, 'class-11'::text
      ),
      (
        421::bigint,
        ...
        4::bigint, 107::bigint, 4::bigint, 33::bigint, '11th'::text, 'class-11'::text
      ),
      (
        422::bigint,
        ...
        4::bigint, 97::bigint, 6::bigint, 33::bigint, '11th'::text, 'class-11'::text
      )
    ) expected(
    ...
    raise exception 'refusing Unacademy fourteenth-batch quality review: reviewed course evidence differs';
  end if;
  ...
  if exists (
    ...
    raise exception 'refusing Unacademy fourteenth-batch quality review: verified teacher identities differ';
  end if;

  if exists (
    select 1
    from (values
      (420::bigint, 34::bigint),
      (421::bigint, 33::bigint),
      (422::bigint, 33::bigint)
  ...
  or (select count(*) from public.playlist_teachers
       where playlist_id in (420, 421, 422)) <> 3 then
    raise exception 'refusing Unacademy fourteenth-batch quality review: course-teacher links differ';
  end if;

  if exists (
    select 1 from public.playlist_quality_reviews
     where playlist_id in (420, 421, 422)
  ) then
    raise exception 'refusing Unacademy fourteenth-batch quality review: target already reviewed';
  end if;
  ...
  or not exists (select 1 from public.chapters where id = 7 and name = 'Friction' and subject_id = 1)
  or not exists (select 1 from public.chapters where id = 107 and name = 'Cell: The Unit of Life' and subject_id = 4)
  or not exists (select 1 from public.chapters where id = 97 and name = 'Anatomy of Flowering Plants' and subject_id = 4) then
    raise exception 'refusing Unacademy fourteenth-batch quality review: reference data differs';
  end if;
  ...
  if v_protected.protected_courses <> 82
    ...
    raise exception 'refusing Unacademy fourteenth-batch quality review: protected JEE mismatch (%)',
...
begin
  update public.playlists p
    ...
    from (values
      (420::bigint, 'NEET: Friction - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Physics | Mahendra Singh'::text),
      (421::bigint, 'NEET: The Unit of Life - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh'::text),
      (422::bigint, 'Plant Anatomy - Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Biology | Pradeep Singh'::text)
  ...
  if v_updated <> 3 then
    raise exception 'Unacademy fourteenth-batch quality review source-title capture mismatch (%)', v_updated;
  end if;
end
...
declare
  ...
  v_note constant text :=
    'Owner-reviewed Unacademy NEET fourteenth-batch faculty evidence; quality transition prepared 2026-08-06.';
begin
  v_result := public.review_playlist_quality(
    420, 'Friction', array[34]::bigint[],
  ...
  if (v_result->>'quality_ready')::boolean is distinct from true
    ...
    raise exception 'course 420 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(
    421, 'Cell: The Unit of Life', array[33]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_note
  );
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 421 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(
    422, 'Anatomy of Flowering Plants', array[33]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_note
  );
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 422 did not become quality-ready (%)', v_result;
  end if;
end
$review$;
...
begin
  if (select count(*) from public.playlists) <> 403
    ...
    raise exception 'Unacademy fourteenth-batch quality review postflight total mismatch';
  end if;

  if exists (
    select 1
    from (values
      (420::bigint, 'Friction'::text, 'NEET: Friction - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Physics | Mahendra Singh'::text, 34::bigint),
      (421::bigint, 'Cell: The Unit of Life'::text, 'NEET: The Unit of Life - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh'::text, 33::bigint),
      (422::bigint, 'Anatomy of Flowering Plants'::text, 'Plant Anatomy - Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Biology | Pradeep Singh'::text, 33::bigint)
    ...
    raise exception 'Unacademy fourteenth-batch quality review course postflight mismatch';
  end if;

  if exists (
    select 1
    from (values
      (420::bigint, 'NEET: Friction - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Physics | Mahendra Singh'::text, 'Friction'::text, 34::bigint),
      (421::bigint, 'NEET: The Unit of Life - Playlist | Class 11 | Unacademy NEET | Live Daily 2.0 | NEET Biology | Pradeep Singh'::text, 'Cell: The Unit of Life'::text, 33::bigint),
      (422::bigint, 'Plant Anatomy - Playlist | Class 11 | Unacademy NEET | LIVE DAILY | NEET Biology | Pradeep Singh'::text, 'Anatomy of Flowering Plants'::text, 33::bigint)
  ...
  or exists (
    ...
    raise exception 'Unacademy fourteenth-batch quality review audit postflight mismatch';
  end if;
  ...
  if v_protected.protected_courses <> 82
     or v_protected.protected_memberships <> 1304
     or v_protected.protected_fingerprint <> '30eee4a4a6842e5beeb7c97083d7f812' then
    raise exception 'Unacademy fourteenth-batch quality review protected JEE mismatch (%)',
      row_to_json(v_protected);
  end if;
end
$postflight$;
