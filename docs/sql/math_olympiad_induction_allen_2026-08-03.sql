...
--
-- Source     : ALLEN JEE — https://www.youtube.com/playlist?list=PL_aKL95N88s3Lagk_JrSrLOK6PFqM2lUk
-- Lessons    : 1
-- Verified   : every youtube_video_id below returned HTTP 200 from YouTube's
--              oEmbed API with author_name "ALLEN JEE", and none was already
--              in the catalogue.
...
begin
  ...
  if exists (select 1 from public.playlists where title = 'Principle of Mathematical Induction — Foundation') then
    raise exception 'course "%" already exists - this file has already been run', 'Principle of Mathematical Induction — Foundation';
  end if;
  ...
  insert into public.playlists (
    ...
    'Principle of Mathematical Induction — Foundation', 'Principle of Mathematical Induction — Foundation', 'Fundamental concepts of mathematical induction, from the official ALLEN JEE Foundation Series.',
  ...
  for v_row in
    select * from (values
      (1, 'iueY3ii7bxQ', 'Principle of Mathematical Induction', 'Mathematics - Fundamental Concepts of Principle of Mathematical Induction | Foundation Series', 5131)
  ...
  if v_inserted <> 1 then
    raise exception 'expected 1 lessons for "%", inserted %', 'Principle of Mathematical Induction — Foundation', v_inserted;
  end if;
  ...
  if exists (
    ...
    raise exception 'a lesson in "%" was left without a goal or class level', 'Principle of Mathematical Induction — Foundation';
  end if;
end $$;
