...
-- production courses 400-404 only. No catalogue rows are created or deleted.
...
begin
  ...
  if exists (
    ...
    from (values
      (
        ...
        2::bigint, array['11th']::text[], '11th'::text, 'class-11'::text,
        6::bigint, 36::bigint
      ),
      (
        401::bigint,
        ...
        1::bigint, array['12th']::text[], '12th'::text, 'class-12'::text,
        11::bigint, 35::bigint
      ),
      (
        402::bigint,
        ...
        4::bigint, array['12th']::text[], '12th'::text, 'class-12'::text,
        5::bigint, 33::bigint
      ),
      (
        403::bigint,
        ...
        4::bigint, array['11th']::text[], '11th'::text, 'class-11'::text,
        7::bigint, 33::bigint
      ),
      (
        404::bigint,
      ...
      legacy_class_levels, audience_focus, class_slug, membership_count, teacher_id
    )
    ...
    where p.id is null
       ...
       or exists (
            ...
            where pv.playlist_id = expected.playlist_id and v.chapter_id is null
          )
  ...
  if exists (
    select 1
    from (values
      (400::bigint, 36::bigint),
      (401::bigint, 35::bigint),
      (402::bigint, 33::bigint),
      (403::bigint, 33::bigint),
      (404::bigint, 33::bigint)
  ...
  or (select count(*) from public.playlist_teachers
       where playlist_id in (400, 401, 402, 403, 404)) <> 5 then
  ...
  if exists (
    select 1 from public.playlist_quality_reviews
     where playlist_id in (400, 401, 402, 403, 404)
  ) then
    raise exception 'refusing Unacademy quality review: target already reviewed';
  end if;
...
begin
  update public.playlists p
    ...
    from (values
      (400::bigint, 'NEET: Hydrogen | Class 11 | Unacademy NEET | Anoop V.'::text),
      (401::bigint, 'NEET: Modern Physics | Class 12 | Live Daily 2.0 | Unacademy NEET | Anu Gupta'::text),
      (402::bigint, 'NEET: Biodiversity & Conservation | LIVE Daily 2.0 | Unacademy NEET | Pradeep Singh'::text),
      (403::bigint, 'NEET: Cell Cycle & Cell Division | Class 11 | Live Daily 2.0 | Unacademy NEET | Pradeep Sir'::text),
      (404::bigint, 'NEET: Microbes in Human Welfare | Class 12 | Unacademy NEET | Pradeep Singh'::text)
...
declare
  v_result jsonb;
  v_sixth_note constant text :=
    'Owner-reviewed official-channel attribution decision 1d0ea7b9-8cac-4f3b-968d-82b4307f264a; Unacademy NEET sixth-batch quality transition prepared 2026-08-05.';
  v_seventh_note constant text :=
    'Owner-reviewed official-channel attribution decision cf45d7d5-43ef-4311-abd7-5297ec2ea3b6; Unacademy NEET seventh-batch quality transition prepared 2026-08-05.';
begin
  v_result := public.review_playlist_quality(400, 'Hydrogen', array[36]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_sixth_note);
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 400 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(401, 'Modern Physics', array[35]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_sixth_note);
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 401 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(402, 'Biodiversity and Conservation', array[33]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_seventh_note);
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 402 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(403, 'Cell Cycle and Cell Division', array[33]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_seventh_note);
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 403 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(404, 'Microbes in Human Welfare', array[33]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_seventh_note);
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 404 did not become quality-ready (%)', v_result;
  end if;
end
$review$;
...
begin
  ...
  if exists (
    select 1
    from (values
      (400::bigint, 'Hydrogen'::text,
       'NEET: Hydrogen | Class 11 | Unacademy NEET | Anoop V.'::text, 36::bigint),
      (401::bigint, 'Modern Physics'::text,
       'NEET: Modern Physics | Class 12 | Live Daily 2.0 | Unacademy NEET | Anu Gupta'::text, 35::bigint),
      (402::bigint, 'Biodiversity and Conservation'::text,
       'NEET: Biodiversity & Conservation | LIVE Daily 2.0 | Unacademy NEET | Pradeep Singh'::text, 33::bigint),
      (403::bigint, 'Cell Cycle and Cell Division'::text,
       'NEET: Cell Cycle & Cell Division | Class 11 | Live Daily 2.0 | Unacademy NEET | Pradeep Sir'::text, 33::bigint),
      (404::bigint, 'Microbes in Human Welfare'::text,
       'NEET: Microbes in Human Welfare | Class 12 | Unacademy NEET | Pradeep Singh'::text, 33::bigint)
  ...
  if exists (
    ...
    from (values
      (400::bigint, 'NEET: Hydrogen | Class 11 | Unacademy NEET | Anoop V.'::text,
       'Hydrogen'::text, 36::bigint, v_sixth_note),
      (401::bigint, 'NEET: Modern Physics | Class 12 | Live Daily 2.0 | Unacademy NEET | Anu Gupta'::text,
       'Modern Physics'::text, 35::bigint, v_sixth_note),
      (402::bigint, 'NEET: Biodiversity & Conservation | LIVE Daily 2.0 | Unacademy NEET | Pradeep Singh'::text,
       'Biodiversity and Conservation'::text, 33::bigint, v_seventh_note),
      (403::bigint, 'NEET: Cell Cycle & Cell Division | Class 11 | Live Daily 2.0 | Unacademy NEET | Pradeep Sir'::text,
       'Cell Cycle and Cell Division'::text, 33::bigint, v_seventh_note),
      (404::bigint, 'NEET: Microbes in Human Welfare | Class 12 | Unacademy NEET | Pradeep Singh'::text,
       'Microbes in Human Welfare'::text, 33::bigint, v_seventh_note)
    ) expected(playlist_id, source_title, title, teacher_id, note)
    left join public.playlist_quality_reviews q on q.playlist_id = expected.playlist_id
    where q.id is null
       or q.note is distinct from expected.note
