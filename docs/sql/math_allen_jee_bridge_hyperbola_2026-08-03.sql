...
--
-- Source     : https://www.youtube.com/playlist?list=PL_aKL95N88s3fFXPHNKWXpvzusNnW0ebg
-- Lessons    : 2
-- Chapters   : 61 Hyperbola
...
begin
  ...
  if exists (select 1 from public.playlists where title = 'Hyperbola — JEE Mathematics') then
    raise exception 'course "%" already exists - this file has already been run', 'Hyperbola — JEE Mathematics';
  end if;

  if exists (
    select 1 from public.videos
    where youtube_video_id = any(array[
      'fhD280t8ps0', 'oF_87NpqzHY'
    ])
  ) then
  ...
  insert into public.playlists (
    ...
    'Hyperbola — JEE Mathematics', 'Hyperbola — JEE Mathematics', 'Hyperbola for JEE, from the official ALLEN JEE channel.',
  ...
  for v_row in
    select * from (values
      (1, 'fhD280t8ps0', 'Hyperbola (Part 2)', 'Hyperbola (Part-2) | Mathematics - Free Bridge Course for JEE Aspirants 📚  ALLEN JEE', 6216, 61),
      (2, 'oF_87NpqzHY', 'Hyperbola (Part 1)', 'Hyperbola (Part-1) | Mathematics - Free Bridge Course for JEE Aspirants 📚  ALLEN JEE', 6177, 61)
  ...
  if v_inserted <> 2 then
    raise exception 'expected 2 lessons for "%", inserted %', 'Hyperbola — JEE Mathematics', v_inserted;
  end if;
  ...
  if exists (
    ...
    raise exception 'a lesson in "%" was left without a goal or class level', 'Hyperbola — JEE Mathematics';
  end if;
end $$;
