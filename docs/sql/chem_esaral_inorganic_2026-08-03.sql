...
--
-- Source     : eSaral — https://www.youtube.com/playlist?list=PLMjEg73ogUEIG8vV3r9yp9BS06haGZTvi
-- Lessons    : 7
-- Chapters   : 5
...
begin
  ...
  if exists (select 1 from public.playlists where title = 'Inorganic Chemistry Mega Revision — eSaral') then
    raise exception 'course "%" already exists - this file has already been run', 'Inorganic Chemistry Mega Revision — eSaral';
  end if;

  if exists (
    select 1 from public.videos
    where youtube_video_id = any(array[
      'oxvVaVkfobE', 'l7_2SGXrDT4', '4x0GRUwgL3A', 'NAzeQd230hs', '4--hL8IngDQ', 'EtbqHb7kM5Q',
      '33L1fqp8ruk'
    ])
  ) then
    raise exception 'at least one of these 7 lessons is already in the catalogue';
  end if;
  ...
  insert into public.playlists (
    ...
    'Inorganic Chemistry Mega Revision — eSaral', 'Inorganic Chemistry Mega Revision — eSaral', 'Inorganic Chemistry revision from the official eSaral channel.',
  ...
  for v_row in
    select * from (values
      (1, 'oxvVaVkfobE', 'd & f block', 'd & f block in One Shot  | Class 12 Chemistry | JEE, NEET | Prateek Sir | eSaral', 3656, 45),
      (2, 'l7_2SGXrDT4', 'Metallurgy (Part 1)', 'JEE Main 2021 | Metallurgy | IOC Mega Revision #5.2 | Quick Revision', 4639, 55),
      (3, '4x0GRUwgL3A', 'Metallurgy (Part 2)', 'Metallurgy One Shot Revision | Class 12 Chemistry | JEE Advanced | Prateek sir', 4178, 55),
      (4, 'NAzeQd230hs', 'p block', 'p block Class 12 Chemistry One Shot | JEE | NEET | eSaral | Prateek Sir', 7276, 50),
      (5, '4--hL8IngDQ', 'Chemical Bonding (Part 1)', 'JEE Main 2021 | Chemical Bonding Part-2 | IOC Mega Revision #2.2 | Quick Revision', 7891, 86),
      (6, 'EtbqHb7kM5Q', 'Chemical Bonding (Part 2)', 'JEE Main 2021 | Chemical Bonding Part-1 | IOC Mega Revision #2.1 | Quick Revision', 7916, 86),
      (7, '33L1fqp8ruk', 'Coordination Compounds', 'JEE Main | Coordination Compounds  | IOC Mega Revision #1 | Quick Revision', 10312, 87)
  ...
  if v_inserted <> 7 then
    raise exception 'expected 7 lessons for "%", inserted %', 'Inorganic Chemistry Mega Revision — eSaral', v_inserted;
  end if;
  ...
  if exists (
    ...
    raise exception 'a lesson in "%" is attributed to a different channel', 'Inorganic Chemistry Mega Revision — eSaral';
  end if;
  ...
  if exists (
    ...
    raise exception 'a lesson in "%" was left without a goal or class level', 'Inorganic Chemistry Mega Revision — eSaral';
  end if;
end $$;
