...
-- Prerequisite: the separately approved twenty-second-batch faculty-link artifact.
...
begin
  if exists (select 1 from public.app_environment) then
    raise exception 'refusing Unacademy twenty-second-batch quality review: target is not production-empty';
  end if;
  if public.catalog_manage_capability()->>'version' is distinct from '11' then
    raise exception 'refusing Unacademy twenty-second-batch quality review: catalogue capability differs';
  end if;
  if (select count(*) from public.playlists) <> 424
    ...
    raise exception 'refusing Unacademy twenty-second-batch quality review: exact baseline differs';
  end if;
  ...
  if exists (
    ...
    from (values
      ...
      (443::bigint, 'Periodic Table'::text, 'Anoop Vashishtha'::text,
       ...
       array['ZmBBuu4-rKU','DjU7kQNy1lM','ZVapFLksVjo','l01Idjq4TeM',
             'u7LCnFAbQDw']::text[])
    ) expected(playlist_id,title,legacy_teacher,source_id,subject_id,chapter_id,
    ...
    raise exception 'refusing Unacademy twenty-second-batch quality review: reviewed course evidence differs';
  end if;

  if exists (
    select 1 from public.playlist_quality_reviews where playlist_id in (441,442,443)
  ) then
    raise exception 'refusing Unacademy twenty-second-batch quality review: target already reviewed';
  end if;
  ...
  select * into v_protected from (
    select
      (select count(*) from public.playlists p where p.id < 167 and exists (
        ...
        join public.learning_goals lg on lg.id = plg.learning_goal_id
        where plg.playlist_id = p.id and lg.slug = 'jee')) protected_courses,
      (select count(*) from public.playlist_videos pv
       join public.playlists p on p.id = pv.playlist_id
       where p.id < 167 and exists (
         select 1 from public.playlist_learning_goals plg
         join public.learning_goals lg on lg.id = plg.learning_goal_id
         where plg.playlist_id = p.id and lg.slug = 'jee')) protected_memberships,
      md5(
  ...
  if v_protected.protected_courses <> 82
    ...
    raise exception 'refusing Unacademy twenty-second-batch quality review: protected JEE mismatch (%)', row_to_json(v_protected);
  end if;
end
$preflight$;
...
begin
  update public.playlists p
    ...
    from (values
      (441::bigint, 'Work, Energy and Power'::text,
       'NEET: Work Energy & Power | Unacademy NEET | Mahendra Singh'::text),
      (442::bigint, 'Solutions'::text,
       'NEET: Solutions - Playlist | Class 12 | Unacademy NEET | Live Daily | NEET Chemistry | Anoop Vashishtha'::text),
      (443::bigint, 'Periodic Table'::text,
       'NEET: Periodic Table | Class 11 | Unacademy NEET | Anoop V.'::text)
    ) expected(playlist_id,display_title,source_title)
   where p.id = expected.playlist_id
     and p.title = expected.display_title
     and p.source_title is null
     and p.source_title_changed is false;

  get diagnostics v_updated = row_count;
  if v_updated <> 3 then
    raise exception 'Unacademy twenty-second-batch quality source-title capture mismatch (%)', v_updated;
  end if;
end
...
declare
  ...
  v_note constant text :=
    'Owner-reviewed Unacademy NEET twenty-second-batch evidence under decision fbf7b3a1-0a19-4dae-b5fe-d967b94f3a7c; quality transition prepared 2026-08-16.';
begin
  v_result := public.review_playlist_quality(
    441, 'Work, Energy and Power', array[34]::bigint[],
  ...
  if (v_result->>'quality_ready')::boolean is distinct from true
    ...
    raise exception 'course 441 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(
    442, 'Solutions', array[36]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_note
  );
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 442 did not become quality-ready (%)', v_result;
  end if;

  v_result := public.review_playlist_quality(
    443, 'Periodic Table', array[36]::bigint[],
    'identified', 'full-course', 'hinglish', 'intermediate', v_note
  );
  if (v_result->>'quality_ready')::boolean is distinct from true
     or v_result->'missing_fields' is distinct from '[]'::jsonb then
    raise exception 'course 443 did not become quality-ready (%)', v_result;
  end if;
end
$review$;
...
begin
  if (select count(*) from public.playlists) <> 424
     ...
     or (select count(*) from public.teacher_learning_goals) <> 37
     or (select count(*) from public.playlist_teachers) <> 179
     or (select count(*) from public.playlist_quality_reviews) <> 50 then
    raise exception 'Unacademy twenty-second-batch quality review postflight total mismatch';
  end if;
  ...
  if exists (
    ...
    left join public.playlists p on p.id = expected.playlist_id
    where p.id is null
       or p.title is distinct from expected.display_title
       ...
       or public.playlist_quality_missing(p.id) is distinct from array[]::text[]
  ) then
    raise exception 'Unacademy twenty-second-batch quality review course postflight mismatch';
  end if;

  if exists (
    select playlist_id from public.playlist_quality_reviews
    where playlist_id in (441,442,443)
    group by playlist_id having count(*) <> 1
  )
  or (select count(*) from public.playlist_quality_reviews
      where playlist_id in (441,442,443)) <> 3 then
    raise exception 'Unacademy twenty-second-batch quality review audit postflight mismatch';
  end if;
  ...
  if v_protected.protected_courses <> 82
     or v_protected.protected_memberships <> 1304
     or v_protected.protected_fingerprint <> '30eee4a4a6842e5beeb7c97083d7f812' then
    raise exception 'Unacademy twenty-second-batch quality review protected JEE mismatch (%)', row_to_json(v_protected);
  end if;
end
$postflight$;

select p.id,p.title,p.source_title,p.title_review_status,
       p.faculty_credit_status,public.playlist_quality_missing(p.id) missing_fields,
