-- Closed-beta staging-only HTTP/JWT fixture preparation helper.
-- It contains no credential and refuses every non-staging database.

begin;

do $forum_beta_http_helper_guard$
...
begin
  ...
  if environment_count <> 1
    ...
    raise exception 'REFUSING: beta HTTP fixture helper requires exactly one staging marker';
  end if;
  if to_regclass('public.forum_beta_members') is null
     or to_regprocedure('public.forum_is_beta_member()') is null
     or to_regprocedure('public.forum_admin_set_beta_member(text,boolean)') is null then
    raise exception 'REFUSING: persistent closed-beta schema is missing';
  end if;
  if public.forum_mode() <> 'off' then
    raise exception 'REFUSING: forum mode must be off before installing fixture helper';
  end if;
  if exists (select 1 from public.forum_beta_members)
     or exists (select 1 from public.forum_posts)
     or exists (select 1 from public.forum_reports) then
    raise exception 'REFUSING: closed-beta staging data is not empty';
  end if;
end;
$forum_beta_http_helper_guard$;
...
begin
  ...
  if not exists (
    ...
    raise exception 'REFUSING: beta HTTP fixture preparation is staging-only';
  end if;
  if public.forum_mode() <> 'off' then
  ...
  if cardinality(p_user_ids) <> 3
     or (select count(distinct user_id) from unnest(p_user_ids) user_id) <> 3 then
    raise exception 'exactly three distinct fixture user ids are required';
  end if;

  select count(*) into fixture_user_count
  from auth.users u
  where u.id = any(p_user_ids)
    and u.email like 'forum-beta-http-' || p_run_id || '-%@staging.invalid'
    and u.raw_app_meta_data ->> 'forum_beta_http_fixture' = p_run_id;
  if fixture_user_count <> 3 then
  ...
  if fixture_profile_count <> 3 then
    raise exception 'three fixture profiles are required';
  end if;

  update auth.users
  set created_at = now() - interval '20 minutes'
  where id = any(p_user_ids)
    and raw_app_meta_data ->> 'forum_beta_http_fixture' = p_run_id;
  ...
  set username = format(
      ...
      full_name = 'Forum beta HTTP staging fixture',
      avatar_url = null,
...
$forum_beta_http_prepare$;

revoke all on function public.forum_stage_prepare_beta_http_fixtures(text,uuid[])
from public, anon, authenticated;
grant execute on function public.forum_stage_prepare_beta_http_fixtures(text,uuid[])
to service_role;
...
select
  ...
  to_regprocedure('public.forum_stage_prepare_beta_http_fixtures(text,uuid[])') is not null
